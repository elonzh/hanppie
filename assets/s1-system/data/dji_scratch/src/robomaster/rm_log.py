...
def logger_init(logger, event_client, file_out_level=DEFAULT_DJI_SCRATCH_FILE_OUT_LEVEL,
                stream_out_level=DEFAULT_DJI_SCRATCH_STREAM_OUT_LEVEL):
    '''
    Args:
    ...
    if stream_out_level != None:
        ...
        stream_out_formatter = logging.Formatter(
            '[%(levelname)-5s]:[%(module)-15.15s]:[%(lineno)-4s]: %(message)s')  # module name is too long
    ...
    if file_out_level != None:
        ...
        file_out_formatter = logging.Formatter(
            '[%(asctime)s]:[%(levelname)-5s]:[%(module)-15s]:[%(lineno)-4s]:[%(funcName)-15s]:%(message)s')
