...
internal fun RemotePage(model: ConsoleModel, modifier: Modifier = Modifier, onBack: (() -> Unit)? = null, expanded: Boolean = false) {
    ...
    BoxWithConstraints(modifier.fillMaxSize().testTag("remote-surface").background(Color(0xff101721)).focusRequester(focus).onFocusChanged {
        ...
        Row(Modifier.align(Alignment.TopEnd).padding(12.dp), horizontalArrangement=Arrangement.spacedBy(8.dp)) {
            onBack?.let { HudButton(if(expanded) tr("返回控制台") else tr("全屏驾驶舱"),if(expanded) tr("控制台") else tr("全屏"),action=it) }
            Text("S1 · ${connected.battery ?: "—"}%", Modifier.background(Color(0xcc152233)).padding(12.dp), color=Color.White)
            HudButton(tr("断开 / 清理会话"), tr("退出"), action=model::disconnect)
        }
        Column(Modifier.align(Alignment.CenterEnd).padding(16.dp), horizontalAlignment=Alignment.CenterHorizontally) {
            Canvas(Modifier.size(48.dp).semantics { contentDescription = tr("底盘相对镜头朝向") }) {
            ...
            Text(if (!enabled) tr("底盘朝向") else if (cameraYaw == null) tr("等待朝向") else tr("镜头前进"), color=Color.White,fontSize=11.sp)
        }
        Column(Modifier.align(Alignment.BottomCenter).fillMaxWidth().padding(16.dp), verticalArrangement=Arrangement.spacedBy(12.dp)) {
            Row(Modifier.align(Alignment.Start), horizontalArrangement=Arrangement.spacedBy(8.dp), verticalAlignment=Alignment.CenterVertically) {
                HudButton(tr("降档"), "−", gear > 1) { model.shiftGear(-1); if(enabled) focus.requestFocus() }
                Text(if(creeping) tr("{0} 档 · 缓行",gear) else tr("{0} 档",gear), color=if(creeping) Color(0xff63e0af) else Color.White)
                HudButton(tr("升档"), "+", gear < DriveSpeed.gears.size) { model.shiftGear(1); if(enabled) focus.requestFocus() }
            }
            Row(Modifier.fillMaxWidth(), horizontalArrangement=Arrangement.SpaceBetween, verticalAlignment=Alignment.Bottom) {
                Stick(tr("底盘"), enabled, { focus.requestFocus() }) { left=it }
                Column(verticalArrangement=Arrangement.spacedBy(8.dp)) {
                    HudButton(tr("切换弹药"), if(gelSelected) tr("水弹 ⇄") else tr("红外 ⇄"), action=model::switchAmmo)
                    HudButton(if(gelSelected) tr("水弹单发") else tr("红外开火"), tr("◎ 开火"), enabled && !connected.busy, model::fireSelected)
                }
                Stick(tr("云台"), enabled, { focus.requestFocus() }) { right=it }
            }
            Row(Modifier.fillMaxWidth(), horizontalArrangement=Arrangement.SpaceBetween, verticalAlignment=Alignment.CenterVertically) {
                Text(if(enabled) tr("● 遥控中") else tr("○ 待机"),color=if(enabled) Color(0xff63e0af) else Color.White)
                if(connected.executionUncertain) HudButton(tr("停止脚本"),tr("停止脚本"),action=model::stop)
                HudButton(if(enabled) tr("停止遥控") else tr("启用遥控"),if(enabled) tr("■ 停止 · Esc") else tr("开始遥控"),!connected.busy) {
                    if(enabled) model.haltRemote() else { focus.requestFocus(); model.enableRemote() }
                }
            }
            if(wide) Text(tr("WASD  移动    Q/E  降/升档    SHIFT  缓行    方向键  云台    R  弹药    SPACE  开火"),color=Color.White.copy(alpha=.65f),fontSize=11.sp)
        }
    }
}
...
@Composable private fun Stick(label: String, enabled: Boolean, claimFocus: () -> Unit, update: (Offset) -> Unit) {
    ...
    Column(horizontalAlignment = Alignment.CenterHorizontally) {
        Canvas(Modifier.size(112.dp).semantics { contentDescription = tr("{0} 摇杆",label) }
