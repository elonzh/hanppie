...
internal fun ChatPage(model: ConsoleModel, modifier: Modifier = Modifier, onVoiceInput: (() -> Unit)? = null, onSettings: () -> Unit = {}) {
    ...
    Column(modifier.fillMaxWidth(), verticalArrangement = Arrangement.spacedBy(10.dp)) {
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
            ...
            Button({ model.replySpeaker.stop(); model.chat.clear() }, enabled = !state.running && state.lines.isNotEmpty()) { Text(tr("新对话")) }
        }
        LazyColumn(Modifier.weight(1f).fillMaxWidth().testTag("chat-messages"), state = list,
            ...
            if (state.lines.isEmpty()) item {
                Column(Modifier.padding(top = 28.dp, bottom = 24.dp)) {
                    Text(tr("说说你想做什么"), fontSize = 24.sp, fontWeight = FontWeight.SemiBold)
                    Text(tr("聊一聊，或一起编写机器人脚本。"), Modifier.padding(top = 8.dp), color = Color(0xff78818d), fontSize = 14.sp)
                }
            }
            items(state.lines) { line ->
                Column(Modifier.fillMaxWidth().background(if (line.role == "我") Color(0xffe9efff) else Color.White,
                    RoundedCornerShape(18.dp)).padding(16.dp)) {
                    Text(tr(line.role), fontSize = 11.sp, color = Color(0xff78818d))
                    SelectionContainer { Text(line.text, Modifier.padding(top = 6.dp), fontSize = if (line.role in listOf("工具", "脚本")) 12.sp else 15.sp,
                        fontFamily = if (line.role == "脚本") FontFamily.Monospace else FontFamily.Default) }
                    if (line.role == "憨皮") Text(tr("朗读"), Modifier.sizeIn(minHeight = 48.dp)
            ...
            if (state.running) item {
                Text(state.streaming.ifBlank { if (state.approval != null) tr("等待确认") else tr("憨皮正在思考…") }, Modifier.padding(12.dp), fontSize = 15.sp)
            }
            state.approval?.let { source -> item {
                Column(Modifier.fillMaxWidth().background(Color.White, RoundedCornerShape(18.dp)).padding(16.dp)) {
                    Text(tr("执行这段脚本？"), fontWeight = FontWeight.SemiBold)
                    SelectionContainer { Text(source, Modifier.padding(vertical = 12.dp), fontSize = 12.sp, fontFamily = FontFamily.Monospace) }
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        Button({ model.chat.approve(false) }) { Text(tr("拒绝")) }
                        Button({ model.chat.approve(true) }, colors = ButtonDefaults.buttonColorsPrimary()) { Text(tr("确认执行")) }
                    }
                }
            } }
        }
        if (speech.speaking) Button(model.replySpeaker::stop) { Text(tr("停止朗读")) }
        state.error?.let { Text(tr(it), color = Color(0xffc64848), fontSize = 12.sp) }
        ...
        if (microphone.active) Text(microphone.partial.ifBlank { if (microphone.processing) tr("正在识别…") else tr("正在听，请说话…") }, fontSize = 13.sp)
        state.elapsedMs?.let { Text(tr("首字 {0} · 本轮 {1}ms",state.firstTokenMs?.let { "${it}ms" } ?: "—",it), fontSize = 11.sp, color = Color(0xff78818d)) }

        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            TextField(input, { if (it.length <= 12000) input = it }, Modifier.weight(1f).testTag("chat-input"), label = tr("发消息"), maxLines = 4, enabled = !microphone.active)

            if (onVoiceInput != null) ComposerIcon(
                if (!microphone.active) tr("语音输入") else if (microphone.processing) tr("取消识别") else tr("结束录音"),
            ...
            ComposerIcon(if (state.running) tr("取消") else tr("发送"), if (state.running) "stop" else "send",
