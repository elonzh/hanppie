"""RoboMaster robot control over the App UDP protocol session."""
...
__all__ = [
    "LED_ARMOR_ALL_MASK",
    "LED_SUBCOMPONENT_ALL",
    "LED_SUBCOMPONENT_BOTTOM",
    "LED_SUBCOMPONENT_TOP",
    "Chassis",
    "Gimbal",
    "GimbalTelemetry",
    "Odometry",
...
def decode_odometry(frame: protocol.DussFrame) -> Odometry | None:
...
def decode_gimbal(frame: protocol.DussFrame) -> GimbalTelemetry | None:
    if (
        not frame.valid
...
class Robot:
    """High-level robot control over the RoboMaster App UDP protocol session."""
    ...
    def __init__(
        ...
        *,
        ...
        self.audio = Audio(self.connection)
        ...
        self._gimbal: GimbalTelemetry | None = None
    ...
    @property
    def odometry(self) -> Odometry | None:
        with self._telemetry_condition:
            return self._odometry

    @property
    def gimbal_telemetry(self) -> GimbalTelemetry | None:
        with self._telemetry_condition:
            return self._gimbal
    ...
    def play_sound(self, sound_id: int, *, timeout: float = 1.0) -> RobotAck:
    ...
    def _handle_duss(self, value: object) -> None:
        if not isinstance(value, protocol.DussFrame):
            return
        telemetry = decode_odometry(value) or decode_gimbal(value)
        if telemetry is not None:
            with self._telemetry_condition:
                if isinstance(telemetry, Odometry):
