...
本次运行依次留下以下证据：第 2～3 节记录发现、机内环境、视频、DSP 和 ADB；第 4～5 节记录 Bridge 就绪与旧解释器问题的定位；第 6 节保留底盘和云台的原始量化测量；第 7 节记录 LED 与红外 controller 返回；第 8 节记录退出后的设备状态。由这些证据支持的当前能力等级统一在 [`architecture.md` 的能力矩阵](./architecture.md#711-当前能力矩阵) 中维护。
