...
UDP 会话只接受指定机器人地址，身份交换与持久 App 会话复用同一核心；非遥控状态发送中性 control 帧，5 秒无匹配数据判定失联。被动发现不主动接管。Lab 上传写入 `python/python_raw.dsp`，用 FTP 成功应答确认传输、DSP MD5 注册；启动/停止仅表示命令已发送，不表示机内任务已完成。启动注册部分失败后禁止重复启动或覆盖上传，必须先停止。
