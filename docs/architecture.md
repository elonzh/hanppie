...
不再通过独立驾驶舱导航页面搬移视频。每次进入已连接、处于前台且没有脚本可能执行的首页，预热视频 5 秒，随后停流并释放解码器，只保留最后一帧，不积累历史回放队列。
预热不进入遥控、不监听音频、不录像。最后一张图像由会话持有的共享 `VideoFrameCache` 保存，所有视频消费者通过同一接口读写，同一连接地址跨页面保留且不设过期时间；不同地址不能读取旧图像，新图像替换旧缓存，关闭应用释放。离开首页、脚本开始或失焦仅停止取流，返回首页或重新激活后重新限时刷新。缓存仅作显示，不能证明机器人当前位置，也不能授权操作。
点击后固定进行 1.35 秒运镜，在第 1 秒开始 600 ms 视频融合；使用已有最后一帧立即衔接，实时解码到达后替换。不因缓存年龄或实时流启动缓慢而拉长动画、退场或提示重试。首次连接从未收到任何画面时保留场景等待首帧，无法凭空保证摄像头画面到达时间。
视频融合后用 400 ms 渐显控制 UI；遥控模式准备在点击进场后与运镜并行，避免 HUD 出现后才触发入场音效；只有最近 500 ms 内有可用实时帧、授权就绪且窗口已激活，才开放驾驶输入。缓存显示期间可退出，实时帧中断会禁用驾驶输入；不显示等待视频或音频的文字。
准备期间不显示独立退出按钮，系统返回仍可取消。
桌面 FFmpeg 进程创建与关闭在后台线程执行，通过完整性筛选的第一张可用画面即报告就绪，不再额外积攒 8 帧。FFmpeg 丢弃损坏包并严格处理解码错误；
按输出帧元数据与解码诊断屏蔽受损帧及依赖它的后续帧，直到无解码错误的完整帧内图像（I 帧）恢复显示，期间保留上一张可用画面。
Android 当前仍使用已渲染帧回调，尚未验证此轮过渡。
普通首页只限时预热视频，不进入遥控；可交互后才接受驾驶输入。退出后重新开始 5 秒预热窗口，关闭监听并结束录像。
退出先归零输入，同时开始 220 ms 控制 UI 淡出、600 ms 视频淡出与 1.15 秒反向运镜；
场景覆盖视频后退出遥控，视频转为首页限时预热。三维资源保持加载，避免返回时空白重建。
用户取消或断线会终止进入流程；离开页面则释放媒体并退出遥控。控制界面失去窗口激活时保持画面和视频，不执行退场；现有 `setForeground(false)` 立即归零、停止持续发射与对讲并撤销输入授权，UI 清空按键与摇杆且禁用按钮。重新激活后恢复输入授权，不恢复先前按住的动作，也不重新播放入场动画。
...
因此升级目标 SDK 时必须重新验证系统方向行为，不能宣称所有系统窗口均可强制锁定。桌面默认 1120×658 横向窗口，最小尺寸为 740×480。普通窗口坐标保存在现有 Preferences DataStore，移动稳定 300 ms 后保存，正常关闭前再次保存；加载偏好并设置位置后才显示窗口，最大化、全屏及最小化不覆盖普通窗口位置。恢复时检查当前屏幕可用区域，原显示器断开则使用系统默认位置。
...
点击控制界面角度指示器后，`GimbalCentering` 使用现有云台速度命令与角度遥测执行回中：俯仰、偏航均以机体相对零度为目标，误差小于等于 1° 停止，速度限于 60°/s，最长 5 秒。任何手动运动输入、失焦、遥控停止或超过 500 ms 的过期反馈都会取消；不新增协议命令，不驱动底盘。该行为复用已验证速度方向，新增回中收敛的实机证据见日期化主页验证记录。
...
- **耐久事件源架构**：每个 Session 独立维护一个 JSONL 文件作为唯一事实源，具备严格的进程锁、文件锁、末事件游标检查与
  fsync 机制；同一会话目录的读写、删除共用固定的 `.sessions.lock`，锁文件数量不随会话增长。
  启动时清理旧的 `<session-id>.lock`；升级前须退出所有旧版进程，不支持新旧版本同时访问同一数据目录。
  目录锁串行化各会话的文件操作，不覆盖模型请求或工具执行；Room SQLite 数据库仅作为可全量重建的高速查询投影；
...
水弹准星反馈由发射指令发送回调驱动，不等待 400 ms 枪口灯收尾；回调仅证明指令已发送，不证明物理激发或命中。连续反馈以最新事件重启动画，避免排队滞后。

普通界面的 Wi-Fi 信号使用共享分档文案（强、中、弱、未知），沿用此前信号条的 UI 阈值：小于 20 为弱，20–39 为中，其余有效原始值为强；该分档未经链路质量标定，不代表吞吐或连接可靠性保证。原始 `0x07:0x09` 无符号字节不解释为 dBm 或百分比，在调试遥测中展示；状态工具字段为 `signalQualityRaw`。操控状态条只使用信号档位图标，不展示信号文字或原始数值；图标占位恒定 20dp，无障碍描述保留本地化等级。状态组之间统一 16dp，组内图标文字为 6dp，夹角数值左对齐。

桌面从主页起通过同一个原生窗口尺寸监听持续保持视频宽高比，计算时扣除标题栏和边框；横向或纵向拖动都按同一比例调整，并保留最小可操作尺寸。进出操控不更换监听、不调整窗口尺寸，避免场景因窗口变化而跳动。视频表面采用填充裁切，消除系统缩放回调期间的短暂比例差造成的黑边。
