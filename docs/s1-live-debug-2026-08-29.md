...
> **文档关系：**本文是一次实机联调的冻结证据，不随代码和能力状态更新；长期维护的当前架构、能力结论和安全边界只见 [`architecture.md`](./architecture.md)。

> **历史命令提示：**本文出现的外部 SDK checkout、外部 LAB-SDK 与 Python 3.8/3.14 环境只表示当时的联调环境，不能作为当前安装说明。当前命令只查看 README，当前实现机制只查看 [`architecture.md`](./architecture.md)。
...
官方相机 API 的 `_stream_sdk(1)` 在本轮返回失败。macOS 缺少 DJI 原生 `libmedia_codec` 只是电脑端导入问题；本轮提供 PyAV 接口兼容模块后，请求仍在开始传输前被机器人拒绝。当前媒体结论见架构文档，本节只保留当时观测。

## 5. 当时使用的工具

下表是本轮执行入口的历史索引，不维护当前 CLI；当前命令以 README 和 `hanppie --help` 为准。

| Hanppie 入口 | 实现 | 用途 | 状态影响 |
| --- | --- | --- | --- |
| `probe-lab` | `lab_bridge.py` | App/Lab 会话与姿态 | 临时进入 Lab，退出时清理 |
| `probe-video` | `lab_video.py` | App 后端 720p 视频 | 临时启停视频流 |
| `adb-enable` | `adb_bootstrap.py` | 从 Lab 开启 root ADB | 改变 USB/ADB 状态，重启恢复 |
| `adb-enable` 内置资源 | [`enable_adb_standalone.py.txt`](../src/hanppie/payloads/enable_adb_standalone.py.txt) | 机内顶层 Popen 载荷 | 同上 |
| `sdk` | `sdk_patch.py` | 临时启用、检查、恢复 SDK 服务 | 停启 DJI 服务并管理 bind mount |
| `probe-connection` | `official_connection.py` | 官方低层握手 | 只建立并关闭会话 |
| `probe-official` | `official_info.py` | 官方高层身份与基础主题 | 进入 SDK/free 模式后关闭 |
| `probe-telemetry` | `official_telemetry.py` | 8 个官方 DDS 主题能力矩阵 | 只订阅/退订 |
| `probe-led` | `official_led.py` | 无机械运动控制验证 | 短暂亮绿灯后关闭 |
| `probe-camera` | `official_camera.py` | 记录官方相机失败边界 | 请求未进入视频传输 |
...
Python 版本不是凭依赖声明推断：本轮离线测试在 CPython 3.8.20 与 3.14.6 各执行 22 项，均通过且当时的主机核心覆盖率约 86%；macOS 上的 Python 3.8.20 还通过固定官方源码 checkout 完成了真机低层握手和 `Robot.initialize()`。Python 3.14 初次高层导入暴露了官方 SDK 对已移除标准库模块 `audioop` 的依赖，当时的环境用 `audioop-lts==0.2.2` 处理。当前 Python 基线只查看 `pyproject.toml` 和 README。
