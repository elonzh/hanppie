...
class Gimbal(object):
    ...
    def set_degree_ctrl(self, pitch_degree, yaw_degree, pitch_accel, yaw_accel, axis_maskbit, coodrdinate, deviation = rm_define.gimbal_deviation):
        yaw_accel = tools.data_limit(yaw_accel, rm_define.gimbal_rotate_speed_min, rm_define.gimbal_rotate_speed_max)
        pitch_accel = tools.data_limit(pitch_accel, rm_define.gimbal_rotate_speed_min, rm_define.gimbal_rotate_speed_max)
        self.msg_buff.init()
        self.task_id = (self.task_id + 1) % duml_cmdset.TASK_ID_MAX
        task_ctrl = duml_cmdset.TASK_FREQ_10Hz << 2 | duml_cmdset.TASK_CTRL_START
...
class Led(object):
    ...
    def set_led(self, comp, mode, ctrl, r, g, b, count, t1, t2, marquee = False):
        ...
        update_states = {'mode' : mode, 'ctrl' : ctrl, 'r' : r, 'g' : g, 'b' : b, 'count' : count, 't1' : t1, 't2' : t2, 'mask' : led_mask}
...
class Vision(object):
    ...
    def line_detection_attr_set(self, attr, value):
        self.msg_buff.init()
...
class Tank(object):
    ...
    def add_gimbal_and_chassis_sub_msg(self, uuid_list, callback):
        ...
        self.msg_buff.append('freq', 'uint16', 5)
        duss_result, resp = self.event_client.send_sync(self.msg_buff)
