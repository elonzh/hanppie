...
class ChassisCtrl(RobotCtrlTool):
    ...
    def set_wheel_speed(self,w2, w1, w3, w4):
        ...
        if self.attr_dict['mode'] != rm_define.chassis_sdk_follow_mode:
            ...
            return get_task_dict(None, 'attr_set', (w1, w2, w3, w4), result = duss_result, err_code=rm_define.BLOCK_ERR_TANKMODE_NOT_SUPPORT)
    ...
    def move_with_time(self, direction_angle, time_wait):
        logger.info('CHASSIS_CTRL: moving with time, direction_angle is ' + str(direction_angle) + ' time is ' + str(time_wait))
        CHECK_VALUE_RANGE_AND_TYPE(direction_angle, -180, 180, int, float)
    ...
    def move_with_distance(self, direction_angle, distance):
        logger.info('CHASSIS_CTRL: moving with time, direction_angle is ' + str(direction_angle) + ' distance is ' + str(distance))
        CHECK_VALUE_RANGE_AND_TYPE(direction_angle, -180, 180, int, float)
        ...
        duss_result, identify = self.chassis.set_position_cmd(0, 0, pos_x, pos_y, 0, int(self.attr_dict['m_speed'] * 100), int(self.attr_dict['r_speed'] * 10))
    ...
    def rotate(self, direction):
        ...
        if self.attr_dict['mode'] != rm_define.chassis_sdk_follow_mode:
            ...
            duss_result = rm_define.DUSS_ERR_NOT_SUPPORT
            logger.warn('CHASSIS_CTRL: cur mode is chassis_sdk_follow_mode, the function is invalid')
            return get_task_dict(None, 'action_immediate', (direction), result = duss_result, err_code=rm_define.BLOCK_ERR_TANKMODE_NOT_SUPPORT)
    ...
    def rotate_with_time(self, direction, time_wait):
        ...
        if self.attr_dict['mode'] != rm_define.chassis_sdk_follow_mode:
            ...
            duss_result = rm_define.DUSS_ERR_NOT_SUPPORT
            logger.warn('CHASSIS_CTRL: cur mode is chassis_sdk_follow_mode, the function is invalid')
            return get_task_dict(None, 'action_with_time', (direction, time_wait), result = duss_result, err_code=rm_define.BLOCK_ERR_TANKMODE_NOT_SUPPORT)
    ...
    def rotate_with_degree(self, direction, degree):
        ...
        if self.attr_dict['mode'] != rm_define.chassis_sdk_follow_mode:
            ...
            duss_result, identify = self.chassis.set_position_cmd(0, 0, 0, 0, degree, int(self.attr_dict['m_speed'] * 100), int(self.attr_dict['r_speed'] * 10))
            ...
            duss_result = rm_define.DUSS_ERR_NOT_SUPPORT
            logger.warn('CHASSIS_CTRL: cur mode is chassis_sdk_follow_mode, the function is invalid')
            return get_task_dict(None, 'no_task', (direction, degree), result = duss_result, err_code=rm_define.BLOCK_ERR_TANKMODE_NOT_SUPPORT)
    ...
    def set_follow_gimbal_offset(self, degree):
        ...
        if self.attr_dict['mode'] != rm_define.chassis_sdk_free_mode:
            ...
            return get_task_dict(None, 'action_immediate', (degree), result = duss_result, err_code=rm_define.BLOCK_ERR_TANKMODE_NOT_SUPPORT)
    ...
    def update_stick_overlay_mode(self):
        if self.attr_dict['stick_overlay_enable'] == rm_define.stick_overlay_and_axes_enable and self.attr_dict['mode'] == rm_define.chassis_sdk_free_mode:
            if self.status_dict['stick_flag'] == True:
                if self.attr_dict['stick_overlay_need_update_mode'] == True:
                    ...
                    if not (self.check_action_state('rotate') or self.check_action_state('move') or self.check_action_state('move_rotate')): #check if stop or not
                        self.attr_dict['stick_overlay_need_update_mode'] = False
                        self._set_chassis_speed(0,0,0)
            else:
                if self.attr_dict['stick_overlay_need_update_mode'] == False:
                    logger.info('CHASSIS_CTRL: enable stick overlay on free mode, and stick invaild, change mode %s '%(str(self.attr_dict['stick_overlay_need_update_mode'])))
                    if not (self.check_action_state('rotate') or self.check_action_state('move') or self.check_action_state('move_rotate')): #check if stop or not
                        self.attr_dict['stick_overlay_need_update_mode'] = True
                        self._set_chassis_stop_depend_mode()
    ...
    @event_register
    def __update_position(self, x, y, yaw_angle, **kw):
        m_speed = rm_define.speed_base_default
        r_speed = rm_define.speed_yaw_default
        ...
        duss_result, identify = self.chassis.set_position_cmd(0, axis_mode, int(x * 100), int(y * 100), int(yaw_angle*10), int(m_speed *100), int(r_speed*10))

        if duss_result == rm_define.DUSS_SUCCESS:
            return get_task_dict(identify, 'task', (x, y, yaw_angle, kw), result = duss_result)
        else:
            self.interrupt_func_unregister()
            return get_task_dict(None, 'no_task', (x, y, yaw_angle, kw), result = duss_result)
    ...
    def update_position(self, x=None, y=None, yaw_angle=None, m_speed=None, r_speed=None, axis_mode=rm_define.axis_mode_default):
        kw = {}
        if m_speed != None:
            kw['m_speed'] = m_speed
        if r_speed != None:
...
class GunCtrl(RobotCtrlTool):
    ...
    def _fire_timer_func(self, *arg, **kw):
        if self.timer_count == 0:
            self.gun.set_cmd_fire(0, self.count)
        self.timer_count += 1
    ...
    def fire_once(self):
        ...
        duss_result = rm_define.DUSS_SUCCESS
        duss_result = self.gun.set_cmd_fire(0, self.count)
        return get_task_dict(None, 'action_immediate', (), result = duss_result)
...
class GimbalCtrl(RobotCtrlTool):
    ...
    def rotate_with_speed(self, yaw_speed, pitch_speed):
        CHECK_VALUE_RANGE_AND_TYPE(yaw_speed, -rm_define.gimbal_rotate_speed_max, rm_define.gimbal_rotate_speed_max, int, float)
        CHECK_VALUE_RANGE_AND_TYPE(pitch_speed, -rm_define.gimbal_rotate_speed_max, rm_define.gimbal_rotate_speed_max, int, float)
        err_code = None
        ...
        if err_code:
            return get_task_dict(None, 'action_immediate', (yaw_speed, pitch_speed), result = duss_result, err_code=err_code)
        else:
    ...
    def rotate_with_degree(self, direction, degree):
        ...
        if direction == rm_define.gimbal_down or direction == rm_define.gimbal_up:
            CHECK_VALUE_RANGE_AND_TYPE(degree, rm_define.gimbal_pitch_degree_ctrl_min, rm_define.gimbal_pitch_degree_ctrl_max, int, float)
        elif direction == rm_define.gimbal_left or direction == rm_define.gimbal_right:
            CHECK_VALUE_RANGE_AND_TYPE(degree, rm_define.gimbal_yaw_degree_ctrl_min, rm_define.gimbal_yaw_degree_ctrl_max, int, float)
        duss_result = rm_define.DUSS_SUCCESS
        self._stop_rotate()
        pitch_degree    = 0
        yaw_degree      = 0
        ...
        if direction == rm_define.gimbal_down:
            degree = tools.data_limit(degree, rm_define.gimbal_pitch_degree_ctrl_min, rm_define.gimbal_pitch_degree_ctrl_max) * 10
            pitch_degree = -degree
            ...
            degree = tools.data_limit(degree, rm_define.gimbal_yaw_degree_ctrl_min, rm_define.gimbal_yaw_degree_ctrl_max) * 10
            yaw_degree = degree
        ...
        if self.attr_dict['mode'] == rm_define.gimbal_yaw_follow_mode and (axis_maskbit & rm_define.gimbal_axis_yaw_maskbit == 1):
        ...
        duss_result, identify = self.gimbal.set_degree_ctrl(pitch_degree, yaw_degree, self.attr_dict['pitch_acc'], self.attr_dict['yaw_acc'], axis_maskbit, rm_define.gimbal_coodrdinate_cur)
    ...
    def pitch_ctrl(self, degree, coordinate = rm_define.gimbal_coodrdinate_ned):
        ...
        CHECK_VALUE_RANGE_AND_TYPE(degree, rm_define.gimbal_pitch_degree_min, rm_define.gimbal_pitch_degree_max, int, float)
        duss_result = rm_define.DUSS_SUCCESS
        self._stop_rotate()
        ...
        duss_result, identify = self.gimbal.set_degree_ctrl(degree, 0, self.attr_dict['pitch_acc'], self.attr_dict['yaw_acc'], rm_define.gimbal_axis_pitch_maskbit, coordinate)
    ...
    def yaw_ctrl(self, degree, coordinate = rm_define.gimbal_coodrdinate_car, follow_flag = False):
        ...
        if coordinate == rm_define.gimbal_coodrdinate_car:
            CHECK_VALUE_RANGE_AND_TYPE(degree, rm_define.gimbal_yaw_degree_min, rm_define.gimbal_yaw_degree_max, int, float)
        else:
            CHECK_VALUE_RANGE_AND_TYPE(degree, rm_define.gimbal_yaw_degree_ctrl_min, rm_define.gimbal_yaw_degree_ctrl_max, int, float)
        duss_result = rm_define.DUSS_SUCCESS
        ...
        if follow_flag or self.attr_dict['mode'] != rm_define.gimbal_yaw_follow_mode:
            ...
            duss_result, identify = self.gimbal.set_degree_ctrl(0, degree, self.attr_dict['pitch_acc'], self.attr_dict['yaw_acc'], rm_define.gimbal_axis_yaw_maskbit, coordinate)
            ...
            logger.warn('GIMBAL_CTRL: cur mode is gimbal_yaw_follow_mode, the function is invalid')
            return get_task_dict(None, 'no_task', (degree, coordinate), result = duss_result, err_code=rm_define.BLOCK_ERR_TANKMODE_NOT_SUPPORT)
    ...
    def angle_ctrl(self, yaw, pitch, coordinate = rm_define.gimbal_coodrdinate_4):
        ...
        CHECK_VALUE_RANGE_AND_TYPE(yaw, rm_define.gimbal_yaw_degree_min, rm_define.gimbal_yaw_degree_ctrl_max, int, float)
        CHECK_VALUE_RANGE_AND_TYPE(pitch, rm_define.gimbal_pitch_degree_min, rm_define.gimbal_pitch_degree_ctrl_max, int, float)
        duss_result = rm_define.DUSS_SUCCESS
        ...
        self._stop_rotate()
        pitch = tools.data_limit(pitch, rm_define.gimbal_pitch_degree_ctrl_min, rm_define.gimbal_pitch_degree_ctrl_max) * 10
        ...
        duss_result, identify = self.gimbal.set_degree_ctrl(pitch, yaw, self.attr_dict['pitch_acc'], self.attr_dict['yaw_acc'], axis_maskbit, coordinate)
    ...
    @event_register
    def __angle_ctrl(self, yaw=None, pitch=None, **kw):
        ...
        if yaw == None and pitch == None:
            return get_task_dict(None, 'no_task', (yaw, pitch, kw), result = duss_result)
        ...
        duss_result, identify = self.gimbal.set_degree_ctrl(pitch, yaw, pitch_speed, yaw_speed, axis_maskbit, coordinate)

        if duss_result == rm_define.DUSS_SUCCESS:
            if err_code:
                return get_task_dict(identify, 'task', (yaw, pitch, kw), result = duss_result, err_code=err_code)
            else:
                return get_task_dict(identify, 'task', (yaw, pitch, kw), result = duss_result)
        else:
            self.interrupt_func_unregister()
            if err_code:
                return get_task_dict(None, 'no_task', (yaw, pitch, kw), result = duss_result, err_code=err_code)
            else:
                return get_task_dict(None, 'no_task', (yaw, pitch, kw), result = duss_result)
    ...
    def __recenter(self, pitch_accel = -1, yaw_accel = -1):
        ...
        if self.attr_dict['mode'] == rm_define.gimbal_yaw_follow_mode:
            axis_maskbit = axis_maskbit &  ~rm_define.gimbal_axis_yaw_maskbit
            logger.warn('GIMBAL_CTRL: cur mode is gimbal_yaw_follow_mode, the yaw ctrl invalid')
            return get_task_dict(None, 'no_task', (pitch_accel, yaw_accel), result = duss_result, err_code=rm_define.BLOCK_ERR_TANKMODE_NOT_SUPPORT)
        self._stop_rotate()
        if pitch_accel == -1:
            pitch_accel = self.attr_dict['pitch_acc']
        if yaw_accel == -1:
    ...
    def compound_motion_ctrl(self, enable_flag, axis, cycle, margin, times=0):
        ...
        margin = margin * 10
        margin = tools.data_limit(margin, rm_define.gimbal_compound_motion_margin_min, rm_define.gimbal_compound_motion_margin_max)
    ...
    def set_speed(self, speed=None, speed2=None):
        if speed != None:
            CHECK_VALUE_RANGE_AND_TYPE(speed, -rm_define.gimbal_rotate_speed_max, rm_define.gimbal_rotate_speed_max, int, float, None)
            speed = tools.data_limit(speed, -rm_define.gimbal_rotate_speed_max, rm_define.gimbal_rotate_speed_max)
            self.attr_dict['pitch_acc'] = speed
        if speed2 != None:
            CHECK_VALUE_RANGE_AND_TYPE(speed2, -rm_define.gimbal_rotate_speed_max, rm_define.gimbal_rotate_speed_max, int, float, None)
    ...
    def update_angle(self, pitch=None, yaw=None, pspeed=None, yspeed=None, coordinate = rm_define.gimbal_coodrdinate_4):
        ...
        result = get_result(self.__angle_ctrl(yaw, pitch, **kw))

    def update_angle_based_on_origin(self, dp=None, dy=None, pspeed=None, yspeed=None):
        self.update_angle(dp, dy, pspeed, yspeed, rm_define.gimbal_coodrdinate_4)

    def update_angle_based_on_cur(self, dp=None, dy=None, pspeed=None, yspeed=None):
        self.update_angle(dp, dy, pspeed, yspeed, rm_define.gimbal_coodrdinate_cur)
...
class LedCtrl(RobotCtrlTool):
    ...
    def set_bottom_led(self, comp, r, g, b, effect):
        logger.info('LED_CTRL: set bottom, comp is %s, r is %s, g is %s, b is %s, effect is %s' %(comp, r, g, b, effect))
        CHECK_VALUE_IN_ENUM_LIST(comp,
                                 **{
                                 'rm_define.armor_bottom_all':rm_define.armor_bottom_all,
                                 'rm_define.armor_bottom_back':rm_define.armor_bottom_back,
    ...
    def set_single_led(self, comp, idx, effect, **kw):
        ...
        if type(idx) == list or isinstance(idx, rm_builtins.RmList):
            ...
            return get_task_dict(None, 'action_immediate', (comp, idx, effect, kw), result = duss_result, err_code=err_code)
...
class ArmorCtrl(RobotCtrlTool):
    ...
    def __init__(self, event_client):
        ...
        self.sdk_event_push_enable = False
    ...
    def sdk_detection_event_push_enable(self, hit_flag=None, reserve=None):
        if hit_flag == 'on':
            self.sdk_event_push_enable = True
            return True
        elif hit_flag == 'off':
            self.sdk_event_push_enable = False
            return True
    ...
    def sdk_detection_event_push_get(self):
        return self.sdk_event_push_enable
    ...
    def hit_event_process(self, event_client, msg):
        ...
        if self.sdk_event_push_callback and self.sdk_event_push_enable:
            self.sdk_event_push_callback(self.hit_index, self.hit_type)
    ...
    def register_event(self, func_dict):
        ...
        if ArmorCtrl.EVENT_HIT_BOTTOM_FRONT_STR in func_dict.keys():
            self.event_client.event_callback_register(ArmorCtrl.ARMOR_BOTTOM_FRONT_STR, func_dict[ArmorCtrl.EVENT_HIT_BOTTOM_FRONT_STR])
        ...
        if ArmorCtrl.EVENT_HIT_BOTTOM_BACK_STR in func_dict.keys():
            self.event_client.event_callback_register(ArmorCtrl.ARMOR_BOTTOM_BACK_STR, func_dict[ArmorCtrl.EVENT_HIT_BOTTOM_BACK_STR])
        ...
        if ArmorCtrl.EVENT_HIT_BOTTOM_LEFT_STR in func_dict.keys():
            self.event_client.event_callback_register(ArmorCtrl.ARMOR_BOTTOM_LEFT_STR, func_dict[ArmorCtrl.EVENT_HIT_BOTTOM_LEFT_STR])
        ...
        if ArmorCtrl.EVENT_HIT_BOTTOM_RIGHT_STR in func_dict.keys():
            self.event_client.event_callback_register(ArmorCtrl.ARMOR_BOTTOM_RIGHT_STR, func_dict[ArmorCtrl.EVENT_HIT_BOTTOM_RIGHT_STR])
        ...
        if ArmorCtrl.EVENT_HIT_TOP_LEFT_STR in func_dict.keys():
            self.event_client.event_callback_register(ArmorCtrl.ARMOR_TOP_LEFT_STR, func_dict[ArmorCtrl.EVENT_HIT_TOP_LEFT_STR])
        ...
        if ArmorCtrl.EVENT_HIT_TOP_RIGHT_STR in func_dict.keys():
            self.event_client.event_callback_register(ArmorCtrl.ARMOR_TOP_RIGHT_STR, func_dict[ArmorCtrl.EVENT_HIT_TOP_RIGHT_STR])
...
class VisionCtrl(RobotCtrlTool):
    event_detection_type_dict = {
        ...
        rm_define.vision_detection_head_shoulder_type:   {rm_define.detection_all_default: 'vision_recognized_head_shoulder'},
    ...
    class ImageInfo(object):
        ...
        def output_format(self):
            return 'type: %s, info %s, distance:%s, x: %f, y:%f, w:%f, h:%f' %(self.type, self.info, self.distance, self.pos.x, self.pos.y, self.size.w, self.size.h)
    ...
    def __init__(self, event_client):
        ...
        self.condition_wait_event_list = {}
        self.condition_mutex = threading.Lock()
        self.wait_event_callback_data_mutex = threading.Lock()
        ...
        if result == rm_define.DUSS_SUCCESS:
            ...
            if result != rm_define.DUSS_SUCCESS:
                ...
                if curr_func & curr_func_enabled != curr_func:
                    logger.error('VISION_CTRL: enable detection func %s failue, cur enable is %s' %(curr_func, curr_func_enabled))
                    return get_task_dict(None, 'action_immediate', (vision_func), result=rm_define.FAILURE, err_code=rm_define.BLOCK_ERR_AI_REFUSED)
        ...
        if num > 0:
            ...
            if detection_type in VisionCtrl.event_detection_type_dict.keys():
                    ...
                    for item in callback_data:
                        if item.distance < self.marker_detection_distance:
                            ...
                            if item.info in VisionCtrl.event_detection_type_dict[detection_type].keys():
                                ...
                                logger.error('VISION_CTRL: detection_typs is %s, info is %s, not support ' %(detection_type, item.info))
                        else:
                            logger.info('VISION_CTRL: %s out of distance, filtered distance is %s, cur detection distance is %s' %(item.type, self.marker_detection_distance, item.distance))
                ...
                if detection_type == rm_define.vision_detection_line_type:
                    ...
                    if VisionCtrl.event_detection_type_dict[detection_type][rm_define.detection_all_default] in self.recognition_event_cb:
                        func_str = VisionCtrl.event_detection_type_dict[detection_type][rm_define.detection_all_default]
                    ...
                    if VisionCtrl.event_detection_type_dict[detection_type][rm_define.detection_all_default] in self.recognition_event_cb:
                        ...
                        logger.info('VISION_CTRL: trigger %s' %VisionCtrl.event_detection_type_dict[detection_type][rm_define.detection_all_default])
                    kind = set()
                    ...
                    for index in kind:
                        if index in VisionCtrl.event_detection_type_dict[detection_type].keys() and VisionCtrl.event_detection_type_dict[detection_type][index] in self.recognition_event_cb:
                            ...
                            logger.info('VISION_CTRL: trigger %s' %VisionCtrl.event_detection_type_dict[detection_type][index])
                    ...
                    logger.warn('VISION_CTRL: not support detection, cur msg detectino is %s, cur support detection is %s' %(detection_type, VisionCtrl.event_detection_type_dict))
            else:
                logger.warn('VISION_CTRL: not support detection, cur msg detectino is %s, cur support detection is %s' %(detection_type, VisionCtrl.event_detection_type_dict))
    ...
    def register_event(self, func_dict):
        ...
        for event_str in VisionCtrl.event_detection_mask_dict.keys():
            if event_str in func_dict.keys():
                ...
                if event_str in VisionCtrl.event_detection_callback_need_data_dict.keys() and VisionCtrl.event_detection_callback_need_data_dict[event_str]:
    ...
    def start_line_follow_until_exception(self):
        ...
        if self.line_detection_flag == False:
            ...
            self.line_detection_flag = True
        self.vision.line_detection_attr_set(1, self.line_follow_color)
        self.line_follow_pause_flag = False
        while True:
            self._wait(0.2)
    ...
    def line_follow_auto_ctrl(self):
        while self.line_follow_auto_ctrl_flag:
            if not self.line_follow_pause_flag and self.is_line_there:
                ...
                self.set_line_follow_param_chassis(rm_define.robot_mode_chassis_follow, chassis_x_ctrl, chassis_y_ctrl, chassis_yaw_ctrl)
            time.sleep(0.03)
    ...
    def line_follow_line_choice(self, direction):
        if direction == rm_define.line_follow_chassis_front:
            return self.vision.line_detection_attr_set(2, 1)
        elif direction == rm_define.line_follow_chassis_left:
            return self.vision.line_detection_attr_set(2, 2)
        elif direction == rm_define.line_follow_chassis_right:
            return self.vision.line_detection_attr_set(2, 3)
    ...
    def get_vision_detection_info(self, detection_type):
        ...
        if len(data_list) > 0:
            ...
            elif detection_type == rm_define.vision_detection_people_type or detection_type == rm_define.vision_detection_car_type and detection_type == data_list[0].type:
...
class MediaCtrl(RobotCtrlTool):
    ...
    def __init__(self, event_client):
        ...
        self.sdk_applause_event_push_enable = False
        self.sdk_applause_event_push_callback = None
    ...
    def recognition_push_process(self, event_client, msg):
        ...
        if recog_type in MediaCtrl.event_detection_type_dict.keys():
             if self.sdk_applause_event_push_callback and self.sdk_applause_event_push_enable:
                self.sdk_applause_event_push_callback(recog_data)
    ...
    def register_event(self, func_dict):
        ...
        for event_str in MediaCtrl.event_detection_mask_dict.keys():
            if event_str in func_dict.keys():
                ...
                if event_str in MediaCtrl.event_detection_callback_need_data_dict.keys() and MediaCtrl.event_detection_callback_need_data_dict[event_str]:
                    self.wait_event_callback_need_data_set_add(event_str)
    ...
    def sdk_detection_event_push_enable(self, applause_flag=None, reserv=None):
        if applause_flag == 'on':
            self.sdk_applause_event_push_enable = True
    ...
    def sdk_applause_event_push_callback_register(self, cb):
        if callable(cb):
            self.sdk_applause_event_push_callback = cb
    ...
    def __play_sound(self, id, wait_for_complete_flag=False):
        logger.info('MEDIA_CTRL: play sound, sound id is %s, wait_for_complete_flag is %s' %(id, wait_for_complete_flag))
    ...
    def play_lib_sound(self, id, wait_for_complete_flag=False):
        logger.info('MEDIA_CTRL: play lib sound, sound id is %s, wait_for_complete_flag is %s' %(id, wait_for_complete_flag))
        if wait_for_complete_flag == True:
            self.interrupt_func_register(self.media.play_sound_task_stop)
...
class RobotCtrl(object):
    ...
    def sub_gimbal_and_chassis_info(self, uuid_list, callback):
        ...
        duss_result, resp = self.robot.add_gimbal_and_chassis_sub_msg(uuid_list, callback)
        if duss_result == rm_define.DUSS_SUCCESS:
...
class ModulesStatusCtrl(object):
    def __init__(self, event_client):
        ...
        self.module_status = {
            0x0100: rm_define.module_status_online,
            0x0104: rm_define.module_status_online,
            0x0300: rm_define.module_status_online,
            0x0400: rm_define.module_status_online,
            0x1700: rm_define.module_status_online,
            0x1801: rm_define.module_status_online,
            0x1802: rm_define.module_status_online,
            0x1803: rm_define.module_status_online,
            0x1804: rm_define.module_status_online,
            0x1805: rm_define.module_status_online,
            0x1806: rm_define.module_status_online,
        }
        self.module_list = []
    ...
    def init(self):
        ...
        self.modulesStatus.event_msg_invalid_check_callback_register(self.check_module_invalid)
    ...
    def check_module_invalid(self, event_msg):
        if self._check_module_status(int(tools.senderid2hostid(event_msg.receiver), 16), rm_define.module_status_offline):
            return True, rm_define.BLOCK_ERR_MODULE_OFFLINE
        elif self._check_module_status(int(tools.senderid2hostid(event_msg.receiver), 16), rm_define.module_status_error):
    ...
    def module_status_process(self, event_client, msg):
        ...
        for i in range(online_module_num):
            module_id = tools.byte_to_uint16(data[0:2])
            online_module_list.append(module_id)
            msg_num = data[2]
            ...
            for n in range(msg_num):
                ...
                if msg_code >> 14 == rm_define.module_msg_type_error:
                   invalid_module_list.append(module_id)
                   break

        offline_module_list = list(set(self.module_list) - set(online_module_list))
