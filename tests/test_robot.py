...
def make_robot(*, return_code: int = 0) -> tuple[Robot, FakeConnection]:
    robot = Robot(robot_ip="192.0.2.10", appid="b6359877")
    connection = FakeConnection(return_code=return_code)
...
def test_direct_robot_requires_arm_and_uses_command_leases(
    ...
    monkeypatch.setattr("hanppie.robot.time.sleep", lambda _seconds: None)
...
def test_gimbal_wire_angles_and_invalid_envelopes() -> None:
    ...
    for length in range(len(payload)):
        assert decode_gimbal(replace(frame, payload=payload[:length])) is None
    for invalid in (
        ...
        assert decode_gimbal(invalid) is None
