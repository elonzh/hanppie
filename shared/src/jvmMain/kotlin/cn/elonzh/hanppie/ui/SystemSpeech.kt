...
internal class SystemSpeech : SpeechEngine {
    ...
    override val state = MutableStateFlow(SpeechState(
        when { command == null -> tr(Res.string.no_system_speech_engine_found)
            command.first().endsWith("spd-say") -> "Speech Dispatcher"
            command.first().endsWith("say") -> tr(Res.string.macos_speech)
            else -> tr(Res.string.windows_speech) }, command != null))

    override fun speak(text: String) {
        require(text.length <= 4000) { tr(Res.string.speech_is_limited_to_4_000_characters_per_reply) }
        if (text.isBlank()) return
        val token = synchronized(lock) {
            generation++
            process?.destroy(); process = null
            state.value = state.value.copy(speaking = command != null, error = if (command == null) tr(Res.string.system_speech_engine_unavailable) else null)
            generation
        }
            ...
            try {
                ...
                synchronized(lock) {
                    if (token == generation) state.value = state.value.copy(speaking = false,
                        error = if (code == 0) null else tr(Res.string.speech_process_exited_value,code))
                }
