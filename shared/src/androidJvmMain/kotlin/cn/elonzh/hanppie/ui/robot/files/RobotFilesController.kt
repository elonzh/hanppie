...
internal class RobotFilesController(
    ...
    private var files: RobotFileSystem? = null
    private var job: Job? = null

    fun attach(files: RobotFileSystem) {
        clear()
        this.files = files
    }
    ...
    fun upload(directory: String, name: String, source: () -> InputStream) =
    ...
    fun download(entry: RobotFileEntry, destination: () -> OutputStream) =
    ...
    fun open(entry: RobotFileEntry, destination: () -> OutputStream, onDownloaded: () -> Unit) =
    ...
    private fun work(operation: UiText, action: suspend (RobotFileSystem) -> RobotFilesState) {
        if (!available()) {
