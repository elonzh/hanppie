...
internal class WorkbenchViewModel(
    ...
    fun openRobotFile(entry: RobotFileEntry) = launchFileAction("open robot file") {
        ...
        val temporary = PlatformFile(directory, "${Uuid.random()}${extension?.let { ".$it" }.orEmpty()}")
        model.robotFiles.open(entry, { temporary.sink().buffered() }) {
            viewModelScope.launch {
                try {
