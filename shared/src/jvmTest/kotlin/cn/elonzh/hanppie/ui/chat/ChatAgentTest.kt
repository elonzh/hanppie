...
class ChatAgentTest {
    ...
    @Test fun continuousConversationUsesCompletedPrompt(): Unit = runBlocking {
        ...
        ChatAgent({ "未连接" }, { error("No robot") }, { error("No robot") }, fake).use { agent ->
    ...
    @Test fun toolResultsReturnToModelAndWaitForExactScriptApproval() = runBlocking {
        ...
        ChatAgent({ "已连接" }, { assertEquals(source, it); executions++; "启动命令已发送" }, { "停止命令已发送" }, fake).use { agent ->
    ...
    @Test fun cancellationNeverExecutesAndKeepsInterruptedContext() = runBlocking {
        ...
        ChatAgent({ "已连接" }, { executions++; "sent" }, { "stop" }, fake).use { agent ->
    ...
    @Test fun denialIsReturnedToModelWithoutExecuting() = runBlocking {
        ...
        ChatAgent({ "已连接" }, { error("Must not execute") }, { error("Must not stop") }, fake).use { agent ->
    ...
    @Test fun httpStatusIsUsefulWithoutLeakingProviderBody(): Unit = runBlocking {
        ...
        ChatAgent({ "disconnected" }, { error("Must not execute") }, { error("Must not stop") }, fake).use { agent ->
    ...
    @Test fun liveCompatibleConversationWithoutRobot() = runBlocking {
        ...
        val config = ModelSettings(apiKey = key!!, model = System.getenv("HANPPIE_LLM_MODEL") ?: config.model)
        ChatAgent({ "测试夹具：未连接机器人，电量未知" }, { error("No robot allowed") }, { error("No robot allowed") }).use { agent ->
