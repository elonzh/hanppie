...
def event_register(func):
    ...
    def wrapper(*args, **kw):
        ...
        # create a task
        robot_task = rm_module.RobotEvent(task_dict['task_identify'], task_dict['event_type'], ctrl)
        ...
        if task_finish == True:
            ...
            elif ctrl.event_client.script_state.get_block_running_fail_reason_code() != 0:
                err_code = ctrl.event_client.script_state.get_block_running_fail_reason_code()
                ctrl.event_client.script_state.reset_block_running_fail_reason_code()
                if 'fail_reason_to_block_running_result_callback' in task_dict['kw'].keys():
                    result = task_dict['kw']['fail_reason_to_block_running_result_callback'](err_code)
...
class RobotCtrlTool(RobotCtrlBases):
    ...
    def robot_sleep_interruptable(self, sleep_time, func_before_event=None, condition_func=None, *args, **kw):
...
class ChassisCtrl(RobotCtrlTool):
    ...
    def __init__(self, event_client):
        ...
        for i in self.sdk_info_push_freq_list:
            self.sdk_info_push_freq_lcm = int(
                i * self.sdk_info_push_freq_lcm / math.gcd(i, self.sdk_info_push_freq_lcm))
    ...
    def set_mode(self, mode):
        ...
        if mode == rm_define.chassis_sdk_free_mode or mode == rm_define.chassis_sdk_follow_mode:
    ...
    def enable_stick_overlay(self):
        duss_result = rm_define.DUSS_SUCCESS
        if self.attr_dict['stick_overlay_enable'] != rm_define.stick_overlay_and_axes_enable:
            logger.info('CHASSIS_CTRL: enable stick overlay, %s' % rm_define.stick_overlay_and_axes_enable)
            self.attr_dict['stick_overlay_enable'] = rm_define.stick_overlay_and_axes_enable
            duss_result = self.chassis.set_stick_overlay(rm_define.stick_overlay_and_axes_enable)
        else:
    ...
    def disable_stick_overlay(self):
        ...
        if self.attr_dict['stick_overlay_enable'] != rm_define.stick_overlay_disable:
            ...
            duss_result = self.chassis.set_stick_overlay(rm_define.stick_overlay_disable)
        else:
    ...
    def set_wheel_speed(self, w2, w1, w3, w4):
        CHECK_VALUE_RANGE_AND_TYPE(w1, rm_define.speed_wheel_min, rm_define.speed_wheel_max, int, float)
        CHECK_VALUE_RANGE_AND_TYPE(w2, rm_define.speed_wheel_min, rm_define.speed_wheel_max, int, float)
        CHECK_VALUE_RANGE_AND_TYPE(w3, rm_define.speed_wheel_min, rm_define.speed_wheel_max, int, float)
        CHECK_VALUE_RANGE_AND_TYPE(w4, rm_define.speed_wheel_min, rm_define.speed_wheel_max, int, float)
        w2 = -w2
        w3 = -w3
        w1 = int(round(w1))
        w2 = int(round(w2))
        ...
        if self.attr_dict['mode'] != rm_define.chassis_sdk_follow_mode:
            ...
            logger.warn('CHASSIS_CTRL: cur mode is chassis_sdk_follow_mode, the function is invalid')
    ...
    def move(self, direction_angle):
        ...
        self.set_action_state('move')
        duss_result = self._move_ctrl(time.time())
        return get_task_dict(None, 'action_immediate', (direction_angle), result=duss_result)

    # continnous action, mutex action
    @event_register
    def move_with_time(self, direction_angle, time_wait):
        logger.info(
            'CHASSIS_CTRL: moving with time, direction_angle is ' + str(direction_angle) + ' time is ' + str(time_wait))
        CHECK_VALUE_RANGE_AND_TYPE(direction_angle, -180, 180, int, float)
        ...
        return get_task_dict(None, 'action_with_time', (direction_angle, time_wait), result=duss_result)

    # continnous action, mutex action
    @event_register
    def move_with_distance(self, direction_angle, distance):
        logger.info(
            'CHASSIS_CTRL: moving with time, direction_angle is ' + str(direction_angle) + ' distance is ' + str(
                distance))
        CHECK_VALUE_RANGE_AND_TYPE(direction_angle, -180, 180, int, float)
        ...
        if self.attr_dict['m_speed'] == 0:
            return get_task_dict(None, 'no_task', (direction_angle, distance), result=duss_result)
        ...
        duss_result, identify = self.chassis.set_position_cmd(0, 0, pos_x, pos_y, 0,
                                                              int(self.attr_dict['m_speed'] * 100),
                                                              int(self.attr_dict['r_speed'] * 10),
                                                              cmd_type=rm_define.TASK)

        if duss_result == rm_define.DUSS_SUCCESS:
            return get_task_dict(identify, 'task', (direction_angle, distance), result=duss_result)
        else:
            self.interrupt_func_unregister()
            return get_task_dict(None, 'no_task', (direction_angle, distance), result=duss_result)
    ...
    def move_with_speed(self, speed_x=0, speed_y=0, speed_z=0):
        logger.info('CHASSIS_CTRL: moveing with speed, speed is %s %s %s' % (speed_x, speed_y, speed_z))
        CHECK_VALUE_RANGE_AND_TYPE(speed_x, rm_define.speed_min, rm_define.speed_max, int, float)
        CHECK_VALUE_RANGE_AND_TYPE(speed_y, rm_define.speed_min, rm_define.speed_max, int, float)
        CHECK_VALUE_RANGE_AND_TYPE(speed_z, rm_define.speed_yaw_min, rm_define.speed_yaw_max, int, float)
        duss_result = rm_define.DUSS_SUCCESS
        self._set_chassis_speed(speed_x, speed_y, speed_z, True)
        self.set_action_state('move')
        return get_task_dict(None, 'action_immediate', (speed_x, speed_y, speed_z), result=duss_result)
    ...
    def move_degree_with_speed(self, speed, degree):
        ...
        self.set_action_state('move')
        duss_result = self._move_ctrl(time.time())
        return get_task_dict(None, 'action_immediate', (speed, degree), result=duss_result)

    # continnous action, mutex action
    @event_register
    def rotate(self, direction):
        logger.info('CHASSIS_CTRL: rotating, diection is' + str(direction))
        CHECK_VALUE_IN_ENUM_LIST(direction,
                                 **{
    ...
    # continnous action, mutex action
    @event_register
    def rotate_with_speed(self, direction, speed):
        logger.info('CHASSIS_CTRL: rotating, diection is' + str(direction) + ' speed is' + str(speed))
        CHECK_VALUE_IN_ENUM_LIST(direction,
                                 **{
                                     'rm_define.clockwise': rm_define.clockwise,
                                     'rm_define.anticlockwise': rm_define.anticlockwise
        ...
        if self.attr_dict['mode'] != rm_define.chassis_sdk_follow_mode:
            self.attr_dict['r_dire'] = direction
            self.set_action_state('rotate')
            duss_result = self._rotate_ctrl(time.time(), speed=speed)
        else:
            duss_result = rm_define.DUSS_ERR_NOT_SUPPORT
            logger.warn('CHASSIS_CTRL: cur mode is chassis_sdk_follow_mode, the function is invalid')
            return get_task_dict(None, 'action_immediate', (direction), result=duss_result,
    ...
    def rotate_with_time(self, direction, time_wait):
        logger.info('CHASSIS_CTRL: rotating with time, direction is ' + str(direction) + ' time is ' + str(time_wait))
        CHECK_VALUE_IN_ENUM_LIST(direction,
                                 **{
                                     'rm_define.clockwise': rm_define.clockwise,
                                     'rm_define.anticlockwise': rm_define.anticlockwise
        ...
        if self.attr_dict['mode'] != rm_define.chassis_sdk_follow_mode:
            ...
            duss_result = rm_define.DUSS_ERR_NOT_SUPPORT
            logger.warn('CHASSIS_CTRL: cur mode is chassis_sdk_follow_mode, the function is invalid')
            return get_task_dict(None, 'action_with_time', (direction, time_wait), result=duss_result,
                                 err_code=rm_define.BLOCK_ERR_TANKMODE_NOT_SUPPORT)
        return get_task_dict(None, 'action_with_time', (direction, time_wait), result=duss_result)
    ...
    def rotate_with_degree(self, direction, degree):
        logger.info('CHASSIS_CTRL: rotating with degree, direction is ' + str(direction) + ' degree is ' + str(degree))
        CHECK_VALUE_IN_ENUM_LIST(direction,
                                 **{
                                     'rm_define.clockwise': rm_define.clockwise,
                                     'rm_define.anticlockwise': rm_define.anticlockwise
        ...
        if self.attr_dict['mode'] != rm_define.chassis_sdk_follow_mode:
            if self.attr_dict['r_speed'] == 0:
                return get_task_dict(None, 'no_task', (direction, degree), result=duss_result)
            self.attr_dict['r_dire'] = direction
            ...
            duss_result, identify = self.chassis.set_position_cmd(0, 0, 0, 0, degree,
                                                                  int(self.attr_dict['m_speed'] * 100),
                                                                  int(self.attr_dict['r_speed'] * 10),
                                                                  cmd_type=rm_define.TASK)

            if duss_result == rm_define.DUSS_SUCCESS:
                return get_task_dict(identify, 'task', (direction, degree), result=duss_result)
            else:
                self.interrupt_func_unregister()
                return get_task_dict(None, 'no_task', (direction, degree), result=duss_result)
        else:
    ...
    def move_and_rotate(self, degree, direction):
        logger.info('CHASSIS_CTRL: move and rotate, direction is ' + str(direction) + 'degree is ' + str(degree))
        CHECK_VALUE_IN_ENUM_LIST(direction,
                                 **{
                                     'rm_define.clockwise': rm_define.clockwise,
                                     'rm_define.anticlockwise': rm_define.anticlockwise
        ...
        if err_code:
            return get_task_dict(None, 'action_immediate', (degree, direction), result=duss_result, err_code=err_code)
        else:
            return get_task_dict(None, 'action_immediate', (degree, direction), result=duss_result)
    ...
    def set_follow_gimbal_offset(self, degree):
        ...
        if self.attr_dict['mode'] != rm_define.chassis_sdk_free_mode:
            ...
            duss_result = rm_define.DUSS_ERR_NOT_SUPPORT
            logger.warn('CHASSIS_CTRL: cur mode is chassis_sdk_free_mode, the function is invalid')
    ...
    def move_with_degree(self, degree, time):
        ...
        duss_result = rm_define.DUSS_SUCCESS
        return get_task_dict(None, 'action_immediate', (degree, time), result=duss_result)
    ...
    def get_position_based_power_on(self, direction=None):
        logger.info('CHASSIS_CTRL: get chassis %s position base power on' % str(direction))
        CHECK_VALUE_IN_ENUM_LIST(direction,
                                 **{
        ...
        if direction == rm_define.chassis_forward:
            return self.status_dict['cur_position_x'] - self.status_dict['init_position_x']
        elif direction == rm_define.chassis_translation:
            return self.status_dict['cur_position_y'] - self.status_dict['init_position_y']
        elif direction == rm_define.chassis_rotate:
            if abs(self.status_dict['cur_attitude_yaw'] - self.status_dict['init_attitude_yaw']) <= 180:
                return self.status_dict['cur_attitude_yaw'] - self.status_dict['init_attitude_yaw']
            elif self.status_dict['cur_attitude_yaw'] - self.status_dict['init_attitude_yaw'] > 180:
                return self.status_dict['cur_attitude_yaw'] - self.status_dict['init_attitude_yaw'] - 360
            elif self.status_dict['cur_attitude_yaw'] - self.status_dict['init_attitude_yaw'] < -180:
                return self.status_dict['cur_attitude_yaw'] - self.status_dict['init_attitude_yaw'] + 360
        elif direction == None:
            return (self.status_dict['cur_position_x'] - self.status_dict['init_position_x'],
                    self.status_dict['cur_position_y'] - self.status_dict['init_position_y'],
                    self.status_dict['cur_attitude_yaw'] - self.status_dict['init_attitude_yaw']
                    )
        else:
    ...
    def get_attitude(self, attitude=None):
        ...
        CHECK_VALUE_IN_ENUM_LIST(attitude,
                                 **{
    ...
    def register_event(self, func_dict):
        if ChassisCtrl.CHASSIS_IMPACT_DETECTION_STR in func_dict.keys():
            logger.info('CHASSIS_CTRL: register callback %s' % ChassisCtrl.CHASSIS_IMPACT_DETECTION_STR)
    ...
    def impact_detection_process(self):
        if self.attitude_status_dict['impact_x_flag'] or self.attitude_status_dict['impact_y_flag']:
            if ChassisCtrl.CHASSIS_IMPACT_DETECTION_STR in self.recognition_event_cb:
                self.event_client.event_come_to_process(ChassisCtrl.CHASSIS_IMPACT_DETECTION_STR)
    ...
    def sdk_info_push_attr_set(self, position_flag=None, pfreq=None, attitude_flag=None, afreq=None, status_flag=None,
                               sfreq=None, freq=None):
        result = False
        ...
        if position_flag == 'on':
            ...
            if pfreq:
                if pfreq in self.sdk_info_push_freq_list:
                    self.sdk_position_info_push_attr['freq_count'] = self.update_attitude_status_unit / pfreq
                    result = True
                else:
                    result = False
            ...
            self.sdk_position_info_push_attr[
                'freq_count'] = self.update_attitude_status_unit / self.sdk_info_push_freq_default
            result = True
        ...
        if attitude_flag == 'on':
            ...
            if afreq:
                if afreq in self.sdk_info_push_freq_list:
                    self.sdk_attitude_info_push_attr['freq_count'] = self.update_attitude_status_unit / afreq
                    result = True
                else:
                    result = False
        ...
        if status_flag == 'on':
            ...
            if sfreq:
                if sfreq in self.sdk_info_push_freq_list:
                    self.sdk_status_info_push_attr['freq_count'] = self.update_attitude_status_unit / sfreq
                    result = True
                else:
                    result = False
        ...
        if freq:
            if freq in self.sdk_info_push_freq_list:
                self.sdk_position_info_push_attr['freq_count'] = (self.update_attitude_status_unit / freq)
                self.sdk_attitude_info_push_attr['freq_count'] = (self.update_attitude_status_unit / freq)
                self.sdk_status_info_push_attr['freq_count'] = (self.update_attitude_status_unit / freq)
                result = True
            else:
                result = False
    ...
    def update_attitude_status(self, status_dict):
        ...
        if self.sdk_info_push_callback:
            if self.sdk_position_info_push_attr['enable_flag'] and self.update_attitude_status_count % int(
                    self.sdk_position_info_push_attr['freq_count']) == 0:
                info['position'] = self.get_position()[0:2]
            if self.sdk_attitude_info_push_attr['enable_flag'] and self.update_attitude_status_count % int(
                    self.sdk_attitude_info_push_attr['freq_count']) == 0:
                info['attitude'] = self.get_attitude()
            if self.sdk_status_info_push_attr['enable_flag'] and self.update_attitude_status_count % int(
                    self.sdk_status_info_push_attr['freq_count']) == 0:
                info['status'] = self.get_status()
            self.sdk_info_push_callback(info)

    def update_stick_overlay_mode(self):
        if self.attr_dict['stick_overlay_enable'] == rm_define.stick_overlay_and_axes_enable and self.attr_dict[
            'mode'] == rm_define.chassis_sdk_free_mode:
            if self.status_dict['stick_flag'] == True:
                if self.attr_dict['stick_overlay_need_update_mode'] == True:
                    logger.info('CHASSIS_CTRL: enable stick overlay on free mode, and stick vaild, change mode')
                    if not (self.check_action_state('rotate') or self.check_action_state(
                            'move') or self.check_action_state('move_rotate')):  # check if stop or not
                        self.attr_dict['stick_overlay_need_update_mode'] = False
    ...
    def _move(self, speed_x, speed_y, func_start, time_sleep):
        ...
        if time_sleep > 0:
            has_condition, has_event = self.robot_sleep_interruptable(time_sleep * 1000, self.stop)
            if not has_event:
                self.stop()
            else:
    ...
    def _rotate_ctrl(self, func_start, t=rm_define.time_forever, speed=None):
        speed_yaw = 0
        if speed == None:
            speed = self.attr_dict['r_speed']
    ...
    def __update_position(self, x, y, yaw_angle, wait_for_complete=True, **kw):
        ...
        if wait_for_complete:
            ...
            duss_result, identify = self.chassis.set_position_cmd(0, axis_mode, int(x * 100), int(y * 100),
                                                                  int(yaw_angle * 10), int(m_speed * 100),
                                                                  int(r_speed * 10), rm_define.TASK)

            if duss_result == rm_define.DUSS_SUCCESS:
                return get_task_dict(identify, 'task', (x, y, yaw_angle, wait_for_complete, kw), result=duss_result)
            else:
                self.interrupt_func_unregister()
                return get_task_dict(None, 'no_task', (x, y, yaw_angle, wait_for_complete, kw), result=duss_result)
        else:
            duss_result, resp = self.chassis.set_position_cmd(0, axis_mode, int(x * 100), int(y * 100),
                                                              int(yaw_angle * 10), int(m_speed * 100),
                                                              int(r_speed * 10), rm_define.NO_TASK)
            return get_task_dict(None, 'action_immediate', (x, y, yaw_angle, wait_for_complete, kw), result=duss_result)

    ########### Python API ###########
    def set_move_speed(self, x=None, y=None, z=None, w2=None, w1=None, w3=None, w4=None):
        if x != None:
    ...
    def update_position(self, x=None, y=None, yaw_angle=None, m_speed=None, r_speed=None,
                        axis_mode=rm_define.axis_mode_default, wait_for_complete=True):
        kw = {}
        if m_speed != None:
            kw['m_speed'] = m_speed
        if r_speed != None:
        ...
        result = get_result(self.__update_position(x, y, -yaw_angle, wait_for_complete=wait_for_complete, **kw))
        return result

    def update_position_based_on_cur(self, x=None, y=None, yaw_angle=None, m_speed=None, r_speed=None,
                                     wait_for_complete=True):
        if wait_for_complete == None:
            wait_for_completet = True
        self.update_position(x, y, yaw_angle, m_speed, r_speed, 0, wait_for_complete=wait_for_complete)

    def update_position_based_on_origin(self, x=None, y=None, yaw_angle=None, m_speed=None, r_speed=None,
                                        wait_for_complete=True):
        if wait_for_complete == None:
            wait_for_completet = True
        self.update_position(x, y, yaw_angle, m_speed, r_speed, 1, wait_for_complete=wait_for_complete)
...
class GunCtrl(RobotCtrlTool):
    ...
    def set_fire_count(self, count=1):
        ...
        duss_result = rm_define.DUSS_SUCCESS
        count = tools.data_limit(count, rm_define.min_fire_count, rm_define.max_fire_count)
        self.count = count
...
class GimbalCtrl(RobotCtrlTool):
    ...
    def set_rotate_speed(self, speed, speed2=None):
        ...
        CHECK_VALUE_RANGE_AND_TYPE(speed, 0, rm_define.gimbal_rotate_speed_max, int, float)
        duss_result = rm_define.DUSS_SUCCESS
        speed = tools.data_limit(speed, rm_define.gimbal_rotate_speed_min, rm_define.gimbal_rotate_speed_max)
        ...
        if self.check_action_state('rotate'):
            ...
            _, _, _, duss_result = self.rotate_with_speed(self.attr_dict['yaw_acc'], self.attr_dict['pitch_acc'])
        return get_task_dict(None, 'attr_set', (speed), result=duss_result)

    ## python and scratch ##
    def get_axis_angle(self, axis=None):
        logger.info('GIMBAL_CTRL: get degree, axis is %s' % str(axis))
        CHECK_VALUE_IN_ENUM_LIST(axis,
                                 **{
    ...
    def _stop_rotate(self):
        duss_result = rm_define.DUSS_SUCCESS
        if self.check_action_state('rotate') or self.check_action_state('rotate_with_speed'):
    ...
    def rotate(self, direction=None):
        ...
        err_code = None
        if self.attr_dict['mode'] == rm_define.gimbal_yaw_follow_mode and yaw_accel != 0:
            logger.warn('GIMBAL_CTRL: cur mode is gimbal_yaw_follow_mode, the yaw ctrl is invalid')
            yaw_accel = 0
            err_code = rm_define.BLOCK_ERR_TANKMODE_NOT_SUPPORT

        duss_result = self.gimbal.set_accel_ctrl(pitch_accel, yaw_accel)
        if err_code:
            return get_task_dict(None, 'action_immediate', (direction), result=duss_result, err_code=err_code)
        else:
            return get_task_dict(None, 'action_immediate', (direction), result=duss_result)

    @event_register
    def rotate_with_speed(self, yaw_speed, pitch_speed):
        CHECK_VALUE_RANGE_AND_TYPE(yaw_speed, -rm_define.gimbal_rotate_speed_max, rm_define.gimbal_rotate_speed_max,
                                   int, float)
        CHECK_VALUE_RANGE_AND_TYPE(pitch_speed, -rm_define.gimbal_rotate_speed_max, rm_define.gimbal_rotate_speed_max,
                                   int, float)
        err_code = None
        if self.attr_dict['mode'] == rm_define.gimbal_yaw_follow_mode and yaw_speed != 0:
            logger.warn('GIMBAL_CTRL: cur mode is gimbal_yaw_follow_mode, the yaw ctrl is invalid')
            yaw_speed = 0
            err_code = rm_define.BLOCK_ERR_TANKMODE_NOT_SUPPORT
        ...
        if err_code:
            return get_task_dict(None, 'action_immediate', (yaw_speed, pitch_speed), result=duss_result,
                                 err_code=err_code)
        else:
            return get_task_dict(None, 'action_immediate', (yaw_speed, pitch_speed), result=duss_result)
    ...
    @event_register
    def rotate_with_degree(self, direction, degree):
        logger.info('GIMBAL_CTRL: rotate with degree, direction is  %s, degree is %f' % (direction, degree))
        CHECK_VALUE_IN_ENUM_LIST(direction,
                                 **{
                                     'rm_define.gimbal_up': rm_define.gimbal_up,
                                     'rm_define.gimbal_down': rm_define.gimbal_down,
        ...
        if direction == rm_define.gimbal_down or direction == rm_define.gimbal_up:
            CHECK_VALUE_RANGE_AND_TYPE(degree, rm_define.gimbal_pitch_degree_ctrl_min,
                                       rm_define.gimbal_pitch_degree_ctrl_max, int, float)
        elif direction == rm_define.gimbal_left or direction == rm_define.gimbal_right:
            CHECK_VALUE_RANGE_AND_TYPE(degree, rm_define.gimbal_yaw_degree_ctrl_min,
                                       rm_define.gimbal_yaw_degree_ctrl_max, int, float)
        duss_result = rm_define.DUSS_SUCCESS
        self._stop_rotate()
        pitch_degree = 0
        yaw_degree = 0
        ...
        if direction == rm_define.gimbal_down:
            degree = tools.data_limit(degree, rm_define.gimbal_pitch_degree_ctrl_min,
                                      rm_define.gimbal_pitch_degree_ctrl_max) * 10
            pitch_degree = -degree
            ...
            degree = tools.data_limit(degree, rm_define.gimbal_yaw_degree_ctrl_min,
                                      rm_define.gimbal_yaw_degree_ctrl_max) * 10
            yaw_degree = degree
        ...
        if self.attr_dict['mode'] == rm_define.gimbal_yaw_follow_mode and (
            ...
            logger.warn('GIMBAL_CTRL: cur mode is gimbal_yaw_follow_mode, the function is invalid')
            return get_task_dict(None, 'no_task', (direction, degree), result=duss_result,
                                 err_code=rm_define.BLOCK_ERR_TANKMODE_NOT_SUPPORT)
        ...
        duss_result, identify = self.gimbal.set_degree_ctrl(pitch_degree, yaw_degree, self.attr_dict['pitch_acc'],
                                                            self.attr_dict['yaw_acc'], axis_maskbit,
                                                            rm_define.gimbal_coodrdinate_cur, cmd_type=rm_define.TASK)
    ...
    def pitch_ctrl(self, degree, coordinate=rm_define.gimbal_coodrdinate_ned):
        ...
        CHECK_VALUE_RANGE_AND_TYPE(degree, rm_define.gimbal_pitch_degree_min, rm_define.gimbal_pitch_degree_max, int,
                                   float)
        duss_result = rm_define.DUSS_SUCCESS
        self._stop_rotate()
        degree = tools.data_limit(degree, rm_define.gimbal_pitch_degree_min, rm_define.gimbal_pitch_degree_max) * 10
        ...
        duss_result, identify = self.gimbal.set_degree_ctrl(degree, 0, self.attr_dict['pitch_acc'],
                                                            self.attr_dict['yaw_acc'],
                                                            rm_define.gimbal_axis_pitch_maskbit, coordinate,
                                                            cmd_type=rm_define.TASK)

        if duss_result == rm_define.DUSS_SUCCESS:
            return get_task_dict(identify, 'task', (degree, coordinate), result=duss_result)
        else:
            self.interrupt_func_unregister()
            return get_task_dict(None, 'no_task', (degree, coordinate), result=duss_result)

    @event_register
    def yaw_ctrl(self, degree, coordinate=rm_define.gimbal_coodrdinate_car, follow_flag=False):
        logger.info('GIMBAL_CTRL: yaw ctrl, degree is %f' % (degree))
        if coordinate == rm_define.gimbal_coodrdinate_car:
            CHECK_VALUE_RANGE_AND_TYPE(degree, rm_define.gimbal_yaw_degree_min, rm_define.gimbal_yaw_degree_max, int,
                                       float)
        else:
            CHECK_VALUE_RANGE_AND_TYPE(degree, rm_define.gimbal_yaw_degree_ctrl_min,
                                       rm_define.gimbal_yaw_degree_ctrl_max, int, float)
        ...
        if follow_flag or self.attr_dict['mode'] != rm_define.gimbal_yaw_follow_mode:
            self._stop_rotate()
            degree = tools.data_limit(degree, rm_define.gimbal_yaw_degree_min, rm_define.gimbal_yaw_degree_max) * 10
            ...
            duss_result, identify = self.gimbal.set_degree_ctrl(0, degree, self.attr_dict['pitch_acc'],
                                                                self.attr_dict['yaw_acc'],
                                                                rm_define.gimbal_axis_yaw_maskbit, coordinate,
                                                                cmd_type=rm_define.TASK)
    ...
    def angle_ctrl(self, yaw, pitch, coordinate=rm_define.gimbal_coodrdinate_4):
        ...
        CHECK_VALUE_RANGE_AND_TYPE(yaw, rm_define.gimbal_yaw_degree_min, rm_define.gimbal_yaw_degree_ctrl_max, int,
                                   float)
        CHECK_VALUE_RANGE_AND_TYPE(pitch, rm_define.gimbal_pitch_degree_min, rm_define.gimbal_pitch_degree_ctrl_max,
                                   int, float)
        duss_result = rm_define.DUSS_SUCCESS
        ...
        if self.attr_dict['mode'] == rm_define.gimbal_yaw_follow_mode:
            ...
            logger.warn('GIMBAL_CTRL: cur mode is gimbal_yaw_follow_mode, the yaw ctrl invalid')
            err_code = rm_define.BLOCK_ERR_TANKMODE_NOT_SUPPORT

        self._stop_rotate()
        pitch = tools.data_limit(pitch, rm_define.gimbal_pitch_degree_ctrl_min,
                                 rm_define.gimbal_pitch_degree_ctrl_max) * 10
        yaw = tools.data_limit(yaw, rm_define.gimbal_yaw_degree_ctrl_min, rm_define.gimbal_yaw_degree_ctrl_max) * 10
        ...
        if self.attr_dict['pitch_acc'] == self.attr_dict['yaw_acc'] == 0:
            self.interrupt_func_unregister()
            return get_task_dict(None, 'no_task', (yaw, pitch, coordinate), result=duss_result)

        duss_result, identify = self.gimbal.set_degree_ctrl(pitch, yaw, self.attr_dict['pitch_acc'],
                                                            self.attr_dict['yaw_acc'], axis_maskbit, coordinate,
                                                            cmd_type=rm_define.TASK)

        if duss_result == rm_define.DUSS_SUCCESS:
            if err_code:
                return get_task_dict(identify, 'task', (yaw, pitch, coordinate), result=duss_result, err_code=err_code)
            else:
                return get_task_dict(identify, 'task', (yaw, pitch, coordinate), result=duss_result)
        else:
            self.interrupt_func_unregister()
            if err_code:
                return get_task_dict(None, 'no_task', (yaw, pitch, coordinate), result=duss_result, err_code=err_code)
            else:
                return get_task_dict(None, 'no_task', (yaw, pitch, coordinate), result=duss_result)
    ...
    def __angle_ctrl(self, yaw=None, pitch=None, wait_for_complete=True, **kw):
        ...
        logger.info('GIMBAL_CTRL: angle ctrl, pitch is %s, yaw is %s' % (str(pitch), (yaw)))
        ...
        if yaw == None and pitch == None:
            return get_task_dict(None, 'action_immediate', (yaw, pitch, wait_for_complete, kw), result=duss_result)
        ...
        if coordinate == rm_define.gimbal_coodrdinate_cur:
            CHECK_VALUE_RANGE_AND_TYPE(yaw, rm_define.gimbal_yaw_degree_ctrl_min, rm_define.gimbal_yaw_degree_ctrl_max,
                                       int, float)
            CHECK_VALUE_RANGE_AND_TYPE(pitch, rm_define.gimbal_pitch_degree_ctrl_min,
                                       rm_define.gimbal_pitch_degree_ctrl_max, int, float)
        else:
            CHECK_VALUE_RANGE_AND_TYPE(yaw, rm_define.gimbal_yaw_degree_min, rm_define.gimbal_yaw_degree_max, int,
                                       float)
            CHECK_VALUE_RANGE_AND_TYPE(pitch, rm_define.gimbal_pitch_degree_min, rm_define.gimbal_pitch_degree_max, int,
                                       float)
        CHECK_VALUE_RANGE_AND_TYPE(pitch_speed, -rm_define.gimbal_rotate_speed_max, rm_define.gimbal_rotate_speed_max,
                                   int, float)
        CHECK_VALUE_RANGE_AND_TYPE(yaw_speed, -rm_define.gimbal_rotate_speed_max, rm_define.gimbal_rotate_speed_max,
        ...
        if wait_for_complete:
            ...
            if self.attr_dict['pitch_acc'] == self.attr_dict['yaw_acc'] == 0:
                self.interrupt_func_unregister()
                return get_task_dict(None, 'no_task', (yaw, pitch, coordinate, wait_for_complete), result=duss_result)

            duss_result, identify = self.gimbal.set_degree_ctrl(pitch, yaw, pitch_speed, yaw_speed, axis_maskbit,
                                                                coordinate, cmd_type=rm_define.TASK)

            if duss_result == rm_define.DUSS_SUCCESS:
                if err_code:
                    return get_task_dict(identify, 'task', (yaw, pitch, wait_for_complete, kw), result=duss_result,
                                         err_code=err_code)
                else:
                    return get_task_dict(identify, 'task', (yaw, pitch, wait_for_complete, kw), result=duss_result)
            else:
                self.interrupt_func_unregister()
                if err_code:
                    return get_task_dict(None, 'no_task', (yaw, pitch, wait_for_complete, kw), result=duss_result,
                                         err_code=err_code)
                else:
                    return get_task_dict(None, 'no_task', (yaw, pitch, wait_for_complete, kw), result=duss_result)
        else:
            duss_result, resp = self.gimbal.set_degree_ctrl(pitch, yaw, pitch_speed, yaw_speed, axis_maskbit,
                                                            coordinate, cmd_type=rm_define.NO_TASK)
            return get_task_dict(None, 'action_immediate', (yaw, pitch, wait_for_complete, kw), result=duss_result)
    ...
    def __recenter(self, pitch_accel=-1, yaw_accel=-1):
        ...
        if self.attr_dict['mode'] == rm_define.gimbal_yaw_follow_mode:
            axis_maskbit = axis_maskbit & ~rm_define.gimbal_axis_yaw_maskbit
            logger.warn('GIMBAL_CTRL: cur mode is gimbal_yaw_follow_mode, the yaw ctrl invalid')
            return get_task_dict(None, 'no_task', (pitch_accel, yaw_accel), result=duss_result,
                                 err_code=rm_define.BLOCK_ERR_TANKMODE_NOT_SUPPORT)
        self._stop_rotate()
        if pitch_accel == -1:
            pitch_accel = self.attr_dict['pitch_acc']
        ...
        self.interrupt_func_register(self.rotate_with_degree_stop)
        self.finished_func_register(self.reset_action_state, 'rotate_with_degree')
        duss_result, identify = self.gimbal.return_middle(axis_maskbit, pitch_accel, 0, yaw_accel)

        if duss_result == rm_define.DUSS_SUCCESS:
            return get_task_dict(identify, 'task', (pitch_accel, yaw_accel), result=duss_result)
        else:
            self.interrupt_func_unregister()
            return get_task_dict(None, 'no_task', (pitch_accel, yaw_accel), result=duss_result)
    ...
    def set_follow_chassis_offset(self, degree):
        ...
        if self.attr_dict['mode'] != rm_define.gimbal_free_mode:
            ...
            logger.warn('GIMBAL_CTRL: cur mode is gimbal_free_mode, the function is invalid')
            return get_task_dict(None, 'action_immediate', (degree), result=duss_result,
                                 err_code=rm_define.BLOCK_ERR_TANKMODE_NOT_SUPPORT)
        return get_task_dict(None, 'action_immediate', (degree), result=duss_result)
    ...
    def compound_motion_ctrl(self, enable_flag, axis, cycle, margin, times=0):
        ...
        margin = margin * 10
        margin = tools.data_limit(margin, rm_define.gimbal_compound_motion_margin_min,
                                  rm_define.gimbal_compound_motion_margin_max)

        duss_result = self.gimbal.set_compound_motion_ctrl(test_flag, enable_flag, axis, phase, cycle, margin, times)
        return get_task_dict(None, 'action_immediate', (enable_flag, axis, cycle, margin, times), result=duss_result)
    ...
    def set_speed(self, speed=None, speed2=None):
        if speed != None:
            CHECK_VALUE_RANGE_AND_TYPE(speed, -rm_define.gimbal_rotate_speed_max, rm_define.gimbal_rotate_speed_max,
                                       int, float, None)
            speed = tools.data_limit(speed, -rm_define.gimbal_rotate_speed_max, rm_define.gimbal_rotate_speed_max)
            self.attr_dict['pitch_acc'] = speed
        if speed2 != None:
            CHECK_VALUE_RANGE_AND_TYPE(speed2, -rm_define.gimbal_rotate_speed_max, rm_define.gimbal_rotate_speed_max,
                                       int, float, None)
            speed2 = tools.data_limit(speed2, -rm_define.gimbal_rotate_speed_max, rm_define.gimbal_rotate_speed_max)
            self.attr_dict['yaw_acc'] = speed2
        return True
    ...
    def update_angle(self, pitch=None, yaw=None, pspeed=None, yspeed=None, coordinate=rm_define.gimbal_coodrdinate_4,
                     wait_for_complete=True):
        kw = {}
        if pspeed != None:
        ...
        result = get_result(self.__angle_ctrl(yaw, pitch, wait_for_complete=wait_for_complete, **kw))

    def update_angle_based_on_origin(self, dp=None, dy=None, pspeed=None, yspeed=None, wait_for_complete=True):
        if wait_for_complete == None:
            wait_for_complete = True
        self.update_angle(dp, dy, pspeed, yspeed, rm_define.gimbal_coodrdinate_4, wait_for_complete=wait_for_complete)

    def update_angle_based_on_cur(self, dp=None, dy=None, pspeed=None, yspeed=None, wait_for_complete=True):
        if wait_for_complete == None:
            wait_for_complete = True
        self.update_angle(dp, dy, pspeed, yspeed, rm_define.gimbal_coodrdinate_cur, wait_for_complete=wait_for_complete)
...
class LedCtrl(RobotCtrlTool):
    ...
    def reset(self):
        ...
        duss_result = rm_define.DUSS_SUCCESS
        duss_result = self.led.set_led(rm_define.armor_all, 1, 7, 0, 127, 70, 0, 1000, 1000)
        return get_task_dict(None, 'action_immediate', (), result=duss_result)

    @event_register
    def set_led(self, comp, r, g, b, effect):
        CHECK_VALUE_IN_ENUM_LIST(comp,
                                 **{
        ...
        CHECK_VALUE_IN_ENUM_LIST(effect,
                                 **{
        ...
        logger.info('LED_CTRL: set, comp is %s, r is %s, g is %s, b is %s, effect is %s' % (comp, r, g, b, effect))
        duss_result = rm_define.DUSS_SUCCESS
        ...
        return get_task_dict(None, 'action_immediate', (comp, r, g, b, effect), result=duss_result)

    @event_register
    def set_top_led(self, comp, r, g, b, effect):
        logger.info('LED_CTRL: set top, comp is %s, r is %s, g is %s, b is %s, effect is %s' % (comp, r, g, b, effect))
        CHECK_VALUE_IN_ENUM_LIST(comp,
                                 **{
                                     'rm_define.armor_top_all': rm_define.armor_top_all,
                                     'rm_define.armor_top_right': rm_define.armor_top_right,
    ...
    def set_bottom_led(self, comp, r, g, b, effect):
        logger.info(
            'LED_CTRL: set bottom, comp is %s, r is %s, g is %s, b is %s, effect is %s' % (comp, r, g, b, effect))
        CHECK_VALUE_IN_ENUM_LIST(comp,
                                 **{
                                     'rm_define.armor_bottom_all': rm_define.armor_bottom_all,
                                     'rm_define.armor_bottom_back': rm_define.armor_bottom_back,
    ...
    def set_single_led(self, comp, idx, effect, **kw):
        logger.info('LED_CTRL: set single, comp is %s, idx is %s, effect is %s' % (comp, str(idx), effect))
        CHECK_VALUE_IN_ENUM_LIST(comp,
                                 **{
                                     'rm_define.armor_top_all': rm_define.armor_top_all,
                                     'rm_define.armor_top_left': rm_define.armor_top_left,
        ...
        if type(idx) == list or isinstance(idx, rm_builtins.RmList):
            idx = list(idx)
            for i in idx:
                CHECK_VALUE_RANGE(i, rm_define.armor_top_led_index_min, rm_define.armor_top_led_index_max)
                if i >= rm_define.armor_top_led_index_min and i <= rm_define.armor_top_led_index_max:
                    mask = mask | 1 << (i - 1)
        elif type(idx) == int:
            CHECK_VALUE_RANGE(idx, rm_define.armor_top_led_index_min, rm_define.armor_top_led_index_max)
            if idx >= rm_define.armor_top_led_index_min and idx <= rm_define.armor_top_led_index_max:
                mask = 1 << (idx - 1)
        else:
            logger.error('LED_CTRL: set single, led index value type error')
            err_code = rm_define.BLOCK_ERR_VALUE_TYPE
            return get_task_dict(None, 'action_immediate', (comp, idx, effect, kw), result=duss_result,
                                 err_code=err_code)
    ...
    def update_led(self, comp, effect, **kw):
        ...
        if effect == rm_define.led_effect.pulse or \
                effect == rm_define.led_effect.scrolling:
            return get_result(self.set_led(comp, r, g, b, effect))
        elif effect == rm_define.led_effect.solid:
            if single_led_index != False:
                return get_result(self.set_single_led(comp, single_led_index, effect, r=r, g=g, b=b))
            else:
                return get_result(self.set_led(comp, r, g, b, effect))
...
class ArmorCtrl(RobotCtrlTool):
    ...
    def hit_event_process(self, event_client, msg):
        ...
        if ArmorCtrl.ARMOR_ALL_STR in self.hit_event_cb:
            ...
            self.event_client.event_come_to_process(ArmorCtrl.ARMOR_ALL_STR, callback_data)
        ...
        if self.hit_index == 2 and ArmorCtrl.ARMOR_BOTTOM_FRONT_STR in self.hit_event_cb:
            self.condition_mutex.acquire()
        ...
        if self.hit_index == 4 and ArmorCtrl.ARMOR_BOTTOM_RIGHT_STR in self.hit_event_cb:
            self.condition_mutex.acquire()
    ...
    def ir_event_process(self, event_client, msg):
        ...
        if ArmorCtrl.IR_TOP_RIGHT_STR in self.hit_event_cb and (self.ir_recv_dev & 0x02):
            self.condition_mutex.acquire()
    ...
    def register_event(self, func_dict):
        if ArmorCtrl.EVENT_HIT_ALL_STR in func_dict.keys():
            self.event_client.event_callback_register(ArmorCtrl.ARMOR_ALL_STR, func_dict[ArmorCtrl.EVENT_HIT_ALL_STR])
            logger.info('ARMOR_CTRL: register event  %s' % (ArmorCtrl.ARMOR_ALL_STR))
        if ArmorCtrl.EVENT_HIT_BOTTOM_FRONT_STR in func_dict.keys():
            self.event_client.event_callback_register(ArmorCtrl.ARMOR_BOTTOM_FRONT_STR,
                                                      func_dict[ArmorCtrl.EVENT_HIT_BOTTOM_FRONT_STR])
            logger.info('ARMOR_CTRL: register event  %s' % (ArmorCtrl.ARMOR_BOTTOM_FRONT_STR))
        if ArmorCtrl.EVENT_HIT_BOTTOM_BACK_STR in func_dict.keys():
            self.event_client.event_callback_register(ArmorCtrl.ARMOR_BOTTOM_BACK_STR,
                                                      func_dict[ArmorCtrl.EVENT_HIT_BOTTOM_BACK_STR])
            logger.info('ARMOR_CTRL: register event  %s' % (ArmorCtrl.ARMOR_BOTTOM_BACK_STR))
        if ArmorCtrl.EVENT_HIT_BOTTOM_LEFT_STR in func_dict.keys():
            self.event_client.event_callback_register(ArmorCtrl.ARMOR_BOTTOM_LEFT_STR,
                                                      func_dict[ArmorCtrl.EVENT_HIT_BOTTOM_LEFT_STR])
            logger.info('ARMOR_CTRL: register event  %s' % (ArmorCtrl.ARMOR_BOTTOM_LEFT_STR))
        if ArmorCtrl.EVENT_HIT_BOTTOM_RIGHT_STR in func_dict.keys():
            self.event_client.event_callback_register(ArmorCtrl.ARMOR_BOTTOM_RIGHT_STR,
                                                      func_dict[ArmorCtrl.EVENT_HIT_BOTTOM_RIGHT_STR])
            logger.info('ARMOR_CTRL: register event  %s' % (ArmorCtrl.ARMOR_BOTTOM_RIGHT_STR))
        if ArmorCtrl.EVENT_HIT_TOP_LEFT_STR in func_dict.keys():
            self.event_client.event_callback_register(ArmorCtrl.ARMOR_TOP_LEFT_STR,
                                                      func_dict[ArmorCtrl.EVENT_HIT_TOP_LEFT_STR])
            logger.info('ARMOR_CTRL: register event  %s' % (ArmorCtrl.ARMOR_TOP_LEFT_STR))
        if ArmorCtrl.EVENT_HIT_TOP_RIGHT_STR in func_dict.keys():
            self.event_client.event_callback_register(ArmorCtrl.ARMOR_TOP_RIGHT_STR,
                                                      func_dict[ArmorCtrl.EVENT_HIT_TOP_RIGHT_STR])
            logger.info('ARMOR_CTRL: register event  %s' % (ArmorCtrl.ARMOR_TOP_RIGHT_STR))
        if ArmorCtrl.EVENT_IR_HIT_TOP_LEFT_STR in func_dict.keys():
            self.event_client.event_callback_register(ArmorCtrl.IR_TOP_LEFT_STR,
        ...
        if ArmorCtrl.EVENT_IR_HIT_TOP_RIGHT_STR in func_dict.keys():
            self.event_client.event_callback_register(ArmorCtrl.IR_TOP_RIGHT_STR,
        ...
        if ArmorCtrl.EVENT_IR_HIT_ALL_STR in func_dict.keys():
            self.event_client.event_callback_register(ArmorCtrl.IR_ALL_STR, func_dict[ArmorCtrl.EVENT_IR_HIT_ALL_STR])
    ...
    def cond_wait(self, func_str):
        ...
        CHECK_VALUE_IN_ENUM_LIST(func_str,
                                 **{
    ...
    def get_last_hit_time(self):
        ...
        if duss_result != rm_define.DUSS_SUCCESS:
            ...
            data = resp['data']
            hit_unixtime = (tools.byte_to_uint32(data[2:6]) << 32 | tools.byte_to_uint32(data[6:10])) / 1000.0
            return hit_unixtime
...
class VisionCtrl(RobotCtrlTool):
    event_detection_type_dict = {
        rm_define.vision_detection_people_type: {rm_define.detection_all_default: 'vision_recognized_people'},
        rm_define.vision_detection_head_shoulder_type: {
            rm_define.detection_all_default: 'vision_recognized_head_shoulder'},
        rm_define.vision_detection_line_type: {rm_define.detection_all_default: 'vision_recognized_line'},
        rm_define.vision_detection_car_type: {rm_define.detection_all_default: 'vision_recognized_car'},
    ...
    event_detection_mask_dict = {
        ...
        'vision_recognized_marker_all_with_follow_line':
    ...
    line_follow_exception_detection_type_list = [
        rm_define.vision_detection_marker_type
    ]
    ...
    class ImageInfo(object):
        ...
        def output_format(self):
            ...
            self.type, self.info, self.distance, self.pos.x, self.pos.y, self.size.w, self.size.h)
    ...
    def __init__(self, event_client):
        ...
        self.line_lost_time_out = rm_define.line_follow_line_lost_distance_default / self.line_follow_front_speed
        self.gimbal_ctrl = GimbalCtrl(event_client)

        self.vision_detection_data = {'data': {
    ...
    def init(self):
        self.line_follow_color_set(rm_define.line_color.blue)
    ...
    def __enable_detection(self, vision_func):
        ...
        CHECK_VALUE_IN_ENUM_LIST(vision_func,
                                 **{
        ...
        if result == rm_define.DUSS_SUCCESS:
            ...
            if result != rm_define.DUSS_SUCCESS:
                ...
                if curr_func & curr_func_enabled != curr_func:
                    logger.error('VISION_CTRL: enable detection func %s failue, cur enable is %s' % (
                    curr_func, curr_func_enabled))
                    return get_task_dict(None, 'action_immediate', (vision_func), result=rm_define.FAILURE,
                                         err_code=rm_define.BLOCK_ERR_AI_REFUSED)
                logger.info('VISION_CTRL: cur enable detection, vision func is %s' % curr_func_enabled)
        else:
    ...
    ## ctrl detection
    def ctrl_detection(self, people=None, pose=None, line=None, marker=None, robot=None, freq=None):
        reset_flag = 0
        if people:
            if self.enable_detection(rm_define.vision_detection_people):
                reset_flag |= rm_define.vision_detection_people
            else:
                self.disable_detection(reset_flag)
                return False
        if pose:
            if self.enable_detection(rm_define.vision_detection_pose):
                reset_flag |= rm_define.vision_detection_pose
            else:
                self.disable_detection(reset_flag)
                return False
        if line:
            if self.enable_detection(rm_define.vision_detection_line):
                reset_flag |= rm_define.vision_detection_line
            else:
                self.disable_detection(reset_flag)
                return False
        if marker:
            if self.enable_detection(rm_define.vision_detection_marker):
                reset_flag |= rm_define.vision_detection_marker
            else:
                self.disable_detection(reset_flag)
                return False
        if robot:
            if self.enable_detection(rm_define.vision_detection_robot):
                reset_flag |= rm_define.vision_detection_robot
            else:
                self.disable_detection(reset_flag)
                return False
    ...
    def attr_set(self, line_color=None, marker_color=None, marker_dist=None):
        result = True
        line_color_enum = {
            'red': rm_define.line_color.red,
            'yellow': rm_define.line_color.yellow,
            'blue': rm_define.line_color.blue
        }

        marker_color_enum = {
            'red': rm_define.marker_color.red,
            'blue': rm_define.marker_color.blue
        }

        if line_color:
            if line_color in line_color_enum.keys():
                result = result and get_result(self.line_follow_color_set(line_color_enum[line_color]))
            else:
                result = result and False

        if marker_color:
            if marker_color in marker_color_enum.keys():
                self.marker_detection_color_set(marker_color_enum[marker_color])
            else:
                result = result and False

        if marker_dist:
            result = result and get_result(self.set_marker_detection_distance(marker_dist))
        return result
        ...
        if num > 0:
            ...
            if detection_type in VisionCtrl.event_detection_type_dict.keys():
                ...
                for i in range(1, num + 1):
                    callback_data.append(self.callback_data_pack(data[9 + 20 * (i - 1):i * 20 + 9], detection_type))
                ...
                if len(VisionCtrl.event_detection_type_dict[detection_type]) > 1:
                    ...
                    for item in callback_data_cp:
                        if item.distance < self.marker_detection_distance:
                            func_str = self.get_marker_xx_all_func_str(detection_type, item.info)
                            if func_str != None:
                                self.condition_wait_list_append(func_str)
                            if item.info in VisionCtrl.event_detection_type_dict[detection_type].keys():
                                func_str = VisionCtrl.event_detection_type_dict[detection_type][item.info]
                                self.condition_wait_list_append(func_str)
                                logger.info('VISION_CTRL: %s wait to wakeup' % (func_str,))
                            else:
                                logger.error('VISION_CTRL: detection_typs is %s, info is %s, not support ' % (
                                detection_type, item.info))
                        else:
                            logger.info(
                                'VISION_CTRL: %s out of distance, filtered distance is %s, cur detection distance is %s' % (
                                item.info, self.marker_detection_distance, item.distance))
                            callback_data.remove(item)

                    if len(callback_data) > 0:
                        func_str = VisionCtrl.event_detection_type_dict[detection_type][rm_define.detection_all_default]
                        self.condition_wait_list_append(func_str)
                else:
                    func_str = VisionCtrl.event_detection_type_dict[detection_type][rm_define.detection_all_default]
                    self.condition_wait_list_append(func_str)
                ...
                if self.sdk_info_push_callback:
                    sdk_info = {}
                    if detection_type == rm_define.vision_detection_people_type:
                        sdk_info['people'] = callback_data
                    elif detection_type == rm_define.vision_detection_pose_type:
                        sdk_info['pose'] = callback_data
                    elif detection_type == rm_define.vision_detection_marker_type:
                        sdk_info['marker'] = callback_data
                    elif detection_type == rm_define.vision_detection_line_type:
                        sdk_info['line'] = callback_data
                    elif detection_type == rm_define.vision_detection_car_type:
                        sdk_info['robot'] = callback_data
                    self.sdk_info_push_callback(sdk_info)
                ...
                if detection_type == rm_define.vision_detection_line_type:
                    ...
                    if VisionCtrl.event_detection_type_dict[detection_type][
                        rm_define.detection_all_default] in self.recognition_event_cb:
                        func_str = VisionCtrl.event_detection_type_dict[detection_type][rm_define.detection_all_default]

                        if func_str in self.wait_event_callback_need_data_set:
                            self.event_client.event_come_to_process(func_str, callback_data)
                        else:
                            self.event_client.event_come_to_process(func_str, None)

                    logger.info('VISION_CTRL: trigger %s' % VisionCtrl.event_detection_type_dict[detection_type][0])
                elif len(VisionCtrl.event_detection_type_dict[detection_type]) > 1:
                    if VisionCtrl.event_detection_type_dict[detection_type][
                        rm_define.detection_all_default] in self.recognition_event_cb:
                        func_str = VisionCtrl.event_detection_type_dict[detection_type][rm_define.detection_all_default]
                        ...
                        logger.info('VISION_CTRL: trigger %s' % VisionCtrl.event_detection_type_dict[detection_type][
                            rm_define.detection_all_default])
                    kind = set()
                    kind_callback_data = {}  # multiple quantitiles of same type. key:kind, value:image list
                    ...
                    for index in kind:
                        if index in VisionCtrl.event_detection_type_dict[detection_type].keys() and \
                                VisionCtrl.event_detection_type_dict[detection_type][
                                    index] in self.recognition_event_cb:
                            func_str = VisionCtrl.event_detection_type_dict[detection_type][index]

                            if func_str in self.wait_event_callback_need_data_set:
                                self.event_client.event_come_to_process(func_str, kind_callback_data[index])
                            else:
                                self.event_client.event_come_to_process(func_str, None)

                            logger.info(
                                'VISION_CTRL: trigger %s' % VisionCtrl.event_detection_type_dict[detection_type][index])

                        # marker_xx_all
                        func_str = self.get_marker_xx_all_func_str(detection_type, index)
    ...
    def register_event(self, func_dict):
        ...
        for event_str in VisionCtrl.event_detection_mask_dict.keys():
            if event_str in func_dict.keys():
                ...
                function_mask = function_mask | VisionCtrl.event_detection_mask_dict[event_str]
                if event_str in VisionCtrl.event_detection_callback_need_data_dict.keys() and \
                        VisionCtrl.event_detection_callback_need_data_dict[event_str]:
                    self.wait_event_callback_need_data_set_add(event_str)

        logger.info('VISION_CTRL: cur register vision event callback is %s' % self.recognition_event_cb)
    ...
    def get_marker_xx_all_func_str(self, detection_type, index):
        if detection_type == rm_define.vision_detection_marker_type:
            if index >= rm_define.marker_number_zero and index <= rm_define.marker_number_nine:
                return VisionCtrl.event_detection_type_dict[detection_type][rm_define.marker_number_all]
            elif index >= rm_define.marker_letter_A and index <= rm_define.marker_letter_Z:
                return VisionCtrl.event_detection_type_dict[detection_type][rm_define.marker_letter_all]
            elif index >= rm_define.marker_trans_left and index <= rm_define.marker_trans_backward:
                return VisionCtrl.event_detection_type_dict[detection_type][rm_define.marker_trans_all]
            else:
                return None
        else:
            return None
    ...
    def line_detection_process(self, data):
        ...
        if num > 1:
            ...
            self.is_line_there = not (abs(output[0][1]) < 0.0001 and abs(output[1][1]) < 0.0001)
        ...
        if self.is_line_there:
            ...
            if cur_time - self.line_there_last_time > self.line_lost_time_out:
                if self.line_lost_first_chassis_stop_flag and not self.line_lost_has_ctrl and self.line_follow_auto_ctrl_flag:
    ...
    def start_line_detection(self):
        ...
        result = self.enable_detection(rm_define.vision_detection_line | rm_define.vision_detection_marker)
        if result == rm_define.DUSS_SUCCESS:
    ...
    def start_line_follow_until_exception(self):
        logger.info('VISION_CTRL: start line follow until exception,  the line color is %s' % color)
        self.cur_marker_with_follow_line_list = []
        self.line_intersection = None
        if self.line_detection_flag == False:
            result = self.start_line_detection()
            if result == rm_define.DUSS_SUCCESS:
                if self.line_follow_auto_ctrl_thread == None or not self.line_follow_auto_ctrl_thread.isAlive():
                    self.line_follow_auto_ctrl_thread = threading.Thread(target=self.line_follow_auto_ctrl)
    ...
    def line_follow_auto_ctrl(self):
        while self.line_follow_auto_ctrl_flag:
            if not self.line_follow_pause_flag and self.is_line_there:
                ...
                gimbal_yaw_ctrl = {'amount': input_error, 'kp': 40, 'ki': 0, 'kd': 0.5, 'max_speed': 1500}
                gimbal_pitch_ctrl = {'amount': 0, 'kp': 0, 'ki': 0, 'kd': 0, 'max_speed': 0}
                chassis_x_ctrl = {'amount': self.line_follow_front_speed, 'kp': 1, 'ki': 0, 'kd': 0, 'max_speed': 2}
                chassis_y_ctrl = {'amount': 0, 'kp': 0.1, 'ki': 0, 'kd': 0, 'max_speed': 1}
                chassis_yaw_ctrl = {'amount': 0, 'kp': 1, 'ki': 0, 'kd': 0, 'max_speed': 30}
                self.set_line_follow_param_gimbal(rm_define.robot_mode_chassis_follow, gimbal_yaw_ctrl,
                                                  gimbal_pitch_ctrl)
                self.set_line_follow_param_chassis(rm_define.robot_mode_chassis_follow, chassis_x_ctrl, chassis_y_ctrl,
                                                   chassis_yaw_ctrl)
            time.sleep(0.03)
    ...
    def stop_line_follow(self):
        ...
        if self.line_detection_flag == True:
            ...
            self.line_follow_chassis_ctrl(rm_define.line_follow_chassis_stop)
            result = self.disable_detection(rm_define.vision_detection_line | rm_define.vision_detection_marker)
            self.line_detection_flag = False
        return get_task_dict(None, 'action_immediate', (), {'result': result})

    def set_line_detection_params(self, ctrl_object, ctrl_amount, ctrl_max_speed, kp, ki=0, kd=0):
        self.vision.ctrl_param_set(ctrl_object, ctrl_amount, ctrl_max_speed, kp, ki, kd)

    def set_line_follow_param_chassis(self, robot_mode, x_ctrl_dict, y_ctrl_dict, yaw_ctrl_dict):
        self.vision.chassis_ctrl_param_set(robot_mode, x_ctrl_dict, y_ctrl_dict, yaw_ctrl_dict)
    ...
    def line_follow_chassis_ctrl(self, direction):
        logger.info('VISION_CTRL: line follow chassis ctrl, the direction is %s' % direction)
        self.line_lost_has_ctrl = True
        if direction == rm_define.line_follow_chassis_stop:
            self.line_follow_pause_flag = True
            gimbal_yaw_ctrl = {'amount': 0, 'kp': 40, 'ki': 0, 'kd': 0.5, 'max_speed': 1500}
            gimbal_pitch_ctrl = {'amount': 0, 'kp': 0, 'ki': 0, 'kd': 0, 'max_speed': 0}
            chassis_x_ctrl = {'amount': 0, 'kp': 1, 'ki': 0, 'kd': 0, 'max_speed': 2}
            chassis_y_ctrl = {'amount': 0, 'kp': 0.1, 'ki': 0, 'kd': 0, 'max_speed': 1}
            chassis_yaw_ctrl = {'amount': 0, 'kp': 1, 'ki': 0, 'kd': 0, 'max_speed': 30}
            self.set_line_follow_param_gimbal(rm_define.robot_mode_chassis_follow, gimbal_yaw_ctrl, gimbal_pitch_ctrl)
            self.set_line_follow_param_chassis(rm_define.robot_mode_chassis_follow, chassis_x_ctrl, chassis_y_ctrl,
                                               chassis_yaw_ctrl)
        elif direction == rm_define.line_follow_chassis_right or direction == rm_define.line_follow_chassis_left or direction == rm_define.line_follow_chassis_front:
    ...
    def line_follow_color_set(self, color):
        CHECK_VALUE_IN_ENUM_LIST(color,
                                 **{
        ...
        return self.vision.detection_attr_set(1, self.line_follow_color)
    ...
    def marker_detection_color_set(self, color):
        ...
        return self.vision.detection_attr_set(2, self.marker_detection_color)
    ...
    def get_line_detection_deviation(self):
        xy_offset_near = self.get_line_detection_deviation_point(rm_define.line_detection_near_point)
        out_x = xy_offset_near[0]
        out_y = xy_offset_near[1]
        return out_x

    def get_line_detection_deviation_point(self, point=rm_define.line_detection_near_point):
        xy_offset_near = (0, 0)
    ...
    def is_cur_line_follow_exception(self, ex):
        logger.info('VISION_CTRL: is cur line follow exception, the check exception is ' + str(ex))
    ...
    def detect_marker_and_aim(self, marker_id):
        ...
        while run_time < 5:  # 2s time out
            ...
            for item in result_info:
                if item.type == rm_define.vision_detection_marker_type and item.info == marker_id:
                    ...
                    gimbal_yaw_ctrl = {'amount': item.pos.x - 0.5, 'kp': 1800, 'ki': 0, 'kd': 1.5, 'max_speed': 250}
                    gimbal_pitch_ctrl = {'amount': 0.53 - item.pos.y, 'kp': 800, 'ki': 0, 'kd': 5.0, 'max_speed': 300}
                    self.set_line_follow_param_gimbal(rm_define.robot_mode_chassis_follow, gimbal_yaw_ctrl,
                                                      gimbal_pitch_ctrl)
                    break
                else:
    ...
    def get_vision_detection_info(self, detection_type):
        ...
        if time.time() - cur_time > 1:
            return output_list
        if len(data_list) > 0:
            if detection_type == rm_define.vision_detection_line_type and detection_type == data_list[0].type:
            ...
            elif detection_type == rm_define.vision_detection_people_type or detection_type == rm_define.vision_detection_car_type and detection_type == \
                    data_list[0].type:
    ...
    def get_mult_line_detection_info(self):
        ...
        if not self.vision_detection_data_mutex.acquire():
            return output_list
        cur_time = self.vision_detection_data['time'][rm_define.vision_detection_line_type]
        data_list = self.vision_detection_data['data'][rm_define.vision_detection_line_type]
        self.vision_detection_data_mutex.release()
...
class MediaCtrl(RobotCtrlTool):
    ...
    def capture(self):
        ...
        if duss_result != rm_define.DUSS_SUCCESS:
            ...
            return get_task_dict(None, 'action_immediate', (), result=duss_result, err_code=err_code)
        return get_task_dict(None, 'action_immediate', (), result=duss_result)
    ...
    def recognition_push_process(self, event_client, msg):
        ...
        recog_type = data[0]
        recog_data = data[1]
        logger.info('MEDIA_CTRL: recog_typ is %s, recog_data is %s' % (recog_type, recog_data))
        if recog_type in MediaCtrl.event_detection_type_dict.keys():
            ...
            if self.sdk_event_push_callback:
                ...
                self.sdk_event_push_callback(event)
            func_str = self.event_detection_type_dict[recog_type][rm_define.detection_all_default]
    ...
    def register_event(self, func_dict):
        ...
        for event_str in MediaCtrl.event_detection_mask_dict.keys():
            if event_str in func_dict.keys():
                ...
                if event_str in MediaCtrl.event_detection_callback_need_data_dict.keys() and \
                        MediaCtrl.event_detection_callback_need_data_dict[event_str]:
                    self.wait_event_callback_need_data_set_add(event_str)

        logger.info('MEDIA_CTRL: cur register vision event callback is %s' % self.recognition_event_cb)
    ...
    def enable_sound_recognition(self, func_mask):
        CHECK_VALUE_IN_ENUM_LIST(func_mask,
                                 **{
        ...
        logger.info('MEDIA_CTRL: enable sound recognition 0x%x, result is %s' % (func_mask, result))
    ...
    def disable_sound_recognition(self, func_mask):
        ...
        if result == rm_define.DUSS_SUCCESS:
            self.recognition_applause = False
        logger.info('MEDIA_CTRL: disable sound recognition 0x%x, result is %s' % (func_mask, result))
        return get_task_dict(None, 'action_immediate', (func_mask), result=result)
    ...
    def disable_all_sound_recognition(self):
        ...
        if self.recognition_applause == True:
            result = self.media.enable_sound_recognition(0, rm_define.sound_detection_applause)
    ...
    def check_condition(self, recog_type):
        CHECK_VALUE_IN_ENUM_LIST(recog_type,
                                 **{
                                     'rm_define.cond_sound_recognized_applause_twice': rm_define.cond_sound_recognized_applause_twice,
                                     'rm_define.cond_sound_recognized_applause_thrice': rm_define.cond_sound_recognized_applause_thrice
                                 })
        ...
        if recog_type in self.condition_event_table.keys():
            ...
            self.condition_event_table[recog_type] = False
        self.condition_mutex.release()
        logger.info('MEDIA_CTRL: check condition, recog_type is %s result is %s' % (recog_type, event_happen))
        return event_happen
    ...
    def record(self, ctrl):
        ...
        if duss_result == rm_define.DUSS_SUCCESS:
            ...
            err_code = self.media.get_err_code()
            err_code = tools.get_block_err_code(rm_define.camera_id, err_code)
            return get_task_dict(None, 'action_immediate', (ctrl), result=duss_result, err_code=err_code)

    @event_register
    def __play_sound(self, id, wait_for_complete_flag=False):
        logger.info(
            'MEDIA_CTRL: play sound, sound id is %s, wait_for_complete_flag is %s' % (id, wait_for_complete_flag))
        if wait_for_complete_flag == True:
            ...
            if duss_result == rm_define.DUSS_SUCCESS:
                return get_task_dict(identify, 'task', (id, wait_for_complete_flag), result=duss_result)
            else:
                self.interrupt_func_unregister()
                return get_task_dict(None, 'no_task', (id, wait_for_complete_flag), result=duss_result)
        else:
            duss_result = self.media.play_sound(id)
            return get_task_dict(None, 'action_immediate', (id, wait_for_complete_flag), result=duss_result)

    @event_register
    def play_lib_sound(self, id, wait_for_complete_flag=False):
        logger.info(
            'MEDIA_CTRL: play lib sound, sound id is %s, wait_for_complete_flag is %s' % (id, wait_for_complete_flag))
        if wait_for_complete_flag == True:
            self.interrupt_func_register(self.media.play_sound_task_stop)
    ...
    @event_register
    def exposure_value_update_sdk(self, ev=None):
        duss_result = rm_define.DUSS_SUCCESS
        if ev == 'default':
            duss_result, resp = self.media.set_camera_ev(rm_define.exposure_value_default)
        elif ev == 'large':
            duss_result, resp = self.media.set_camera_ev(rm_define.exposure_value_large)
        elif ev == 'medium':
            duss_result, resp = self.media.set_camera_ev(rm_define.exposure_value_medium)
        elif ev == 'small':
            duss_result, resp = self.media.set_camera_ev(rm_define.exposure_value_small)
        else:
            duss_result = rm_define.DUSS_ERR_FAILURE
        return get_task_dict(None, 'action_immediate', (ev), result=duss_result)
    ...
    def zoom_value_update(self, zv):
        zv = int(zv * 100)
        CHECK_VALUE_RANGE_AND_TYPE(zv, rm_define.zoom_value_min, rm_define.zoom_value_max, int)
    ...
    def stop(self):
        ...
        self.media.play_sound_task_stop()
        duss_result = rm_define.DUSS_SUCCESS
        if self.recording:
            self.record(0)
    ...
    def play_sound(self, id, **kw):
        CHECK_VALUE_IN_ENUM_LIST(id,
                                 **{
                                     ...
                                     'rm_define.media_custom_audio_0': rm_define.media_custom_audio_0,
                                     'rm_define.media_custom_audio_1': rm_define.media_custom_audio_1,
                                     'rm_define.media_custom_audio_2': rm_define.media_custom_audio_2,
                                     'rm_define.media_custom_audio_3': rm_define.media_custom_audio_3,
                                     'rm_define.media_custom_audio_4': rm_define.media_custom_audio_4,
                                     'rm_define.media_custom_audio_5': rm_define.media_custom_audio_5,
                                     'rm_define.media_custom_audio_6': rm_define.media_custom_audio_6,
                                     'rm_define.media_custom_audio_7': rm_define.media_custom_audio_7,
                                     'rm_define.media_custom_audio_8': rm_define.media_custom_audio_8,
                                     'rm_define.media_custom_audio_9': rm_define.media_custom_audio_9,
                                 })
...
class RobotCtrl(object):
    ...
    def __init__(self, event_client, chassis, gimbal, servo=None):
        ...
        self.servo_status_dict = {}
        self.battery_status_dict = {}
        self.msg_id = None
    ...
    def set_mode(self, mode):
        ...
        if type(mode) == str:
            if mode == 'gimbal_lead':
                ...
                return False
        CHECK_VALUE_RANGE_AND_TYPE(mode, rm_define.robot_mode_free, rm_define.robot_mode_gimbal_follow, int)
        logger.info('TANK_CTRL: set work mode, the mode is %s' % mode)
        if mode == rm_define.robot_mode_gimbal_follow or \
                mode == rm_define.robot_mode_chassis_follow or \
                mode == rm_define.robot_mode_free:
    ...
    def get_battery_percentage(self):
        if 'capacity_percentage' in self.battery_status_dict.keys():
            return self.battery_status_dict['capacity_percentage']
    ...
    def init(self):
        ...
        self.sub_gimbal_and_chassis_info(uuid_list, self.gimbal_and_chassis_push_info_process)

    def sub_gimbal_and_chassis_info(self, uuid_list, callback):
        logger.info('TANK_CTRL: sub gimbal and chassis info')
        duss_result, resp = self.robot.add_gimbal_and_chassis_sub_msg(RobotCtrl.GIMBAL_CHASSIS_SUB_INFO_PUSH_FREQ,
                                                                      uuid_list, callback)
        if duss_result == rm_define.DUSS_SUCCESS:
            if resp['data'][0] == 0 or resp['data'][0] == 0x23:
                ...
                logger.error('TANK_CTRL: sub gimbal and chassis info error. error code is 0x%x' % resp['data'][0])
        else:
            logger.error('TANK_CTRL: sub gimbal and chassis info error. error code is %s' % duss_result)
        return duss_result, resp

    def unsub_gimbal_and_chassis_info(self):
        if self.msg_id != None:
            logger.info('TANK_CTRL: unsub gimbal and chassis info msg id is %s' % self.msg_id)
    ...
    def gimbal_and_chassis_push_info_process(self, event_client, msg):
        ...
        stick_flag = data[121]
        servo_info = data[122:139]
        battery_info = data[139:149]

        self.chassis_status_dict['cur_speed_wheel1'] = tools.byte_to_int16(esc_info[0:2])
        self.chassis_status_dict['cur_speed_wheel2'] = tools.byte_to_int16(esc_info[2:4])
        self.chassis_status_dict['cur_speed_wheel3'] = tools.byte_to_int16(esc_info[4:6])
        self.chassis_status_dict['cur_speed_wheel4'] = tools.byte_to_int16(esc_info[6:8])
        ...
        self.chassis_status_dict['cur_attitude_yaw'] = tools.byte_to_float(attitude_info[0:4])
        self.chassis_status_dict['cur_attitude_pitch'] = tools.byte_to_float(attitude_info[4:8])
        self.chassis_status_dict['cur_attitude_roll'] = tools.byte_to_float(attitude_info[8:12])
        ...
        self.chassis_status_dict['stick_flag'] = stick_flag & 0x01 | self.gimbal_status_dict['stick_flag']

        if self.init_chassis_pos_flag == 0:
            self.init_chassis_pos_flag = 1
            self.chassis_status_dict['init_position_x'] = self.chassis_status_dict['cur_position_x']
            self.chassis_status_dict['init_position_y'] = self.chassis_status_dict['cur_position_y']
            self.chassis_status_dict['init_attitude_yaw'] = self.chassis_status_dict['cur_attitude_yaw']

        self.battery_status_dict['ad_vol'] = tools.byte_to_uint16(battery_info[0:2])
        self.battery_status_dict['temperature'] = tools.byte_to_int16(battery_info[2:4])
        self.battery_status_dict['current'] = tools.byte_to_int32(battery_info[4:8])
        self.battery_status_dict['capacity_percentage'] = tools.byte_to_uint8(battery_info[8:9])

        servo_id = servo_info[0] & 0x0f
        for i in range(servo_id.bit_length()):
            if ((servo_id >> i) & 0x01) == 1:
                self.servo_status_dict[i + 1] = {
                    'speed': tools.byte_to_int16(servo_info[i * 2 + 1:i * 2 + 3]),
                    'angle': tools.byte_to_int16(servo_info[i * 2 + 9:i * 2 + 11])
                }
        ...
        if self.servo:
            self.servo.update_status(self.servo_status_dict)
...
class MobileCtrl(object):
    ...
    def info_push_process(self, event_client, msg):
        ...
        while type_pos < len(data):
            ...
            if mobile_info_type in self.mobile_type_info_parse.keys():
                ...
                logger.warn('MOBILE_CTRL:no parse key in mobile info push process, the key is %s' % mobile_info_type)
                break
            type_pos += length
    ...
    def enable_info_push(self):
        info_id = rm_define.mobile_info_gps_id | rm_define.mobile_info_accel_id | rm_define.mobile_info_gyro_id | rm_define.mobile_info_atti_id
...
class LogCtrl(object):
    ...
    def info_msg(self, *msg_args):
        return self.send_string(rm_define.custom_msg_type_debug, rm_define.custom_msg_level_info, msg_args)

    def debug_msg(self, *msg_args):
        return self.send_string(rm_define.custom_msg_type_debug, rm_define.custom_msg_level_debug, msg_args)

    def error_msg(self, *msg_args):
        return self.send_string(rm_define.custom_msg_type_debug, rm_define.custom_msg_level_error, msg_args)

    def fatal_msg(self, *msg_args):
        return self.send_string(rm_define.custom_msg_type_debug, rm_define.custom_msg_level_fatal, msg_args)

    def print_msg(self, *msg_args):
        return self.send_string(rm_define.custom_msg_type_debug, rm_define.custom_msg_level_none, msg_args)

    def show_msg(self, *msg_args):
        return self.send_string(rm_define.custom_msg_type_show, rm_define.custom_msg_level_none, msg_args)
...
class RobotTools(RobotCtrlTool):
    ...
    def get_timer_count(self):
        if self.timer_state == rm_define.timer_start:
            return math.ceil((time.time() - self.timer_start_stamp + self.timer_total) * 1000) / 1000
        else:
    ...
    def timer_ctrl(self, ctrl):
        ...
        if ctrl == rm_define.timer_start:
            ...
            if self.timer_state == rm_define.timer_start:
                self.timer_total = self.timer_total + (time.time() - self.timer_start_stamp)
                self.timer_start_stamp = 0
                self.timer_state = rm_define.timer_stop
    ...
    def get_localtime(self, time_unit):
        CHECK_VALUE_IN_ENUM_LIST(time_unit,
                                 **{
    ...
    def get_unixtime(self):
        if self.time_offset == 0:
            ...
            data = resp['data']
            unixtime = (tools.byte_to_uint32(data[1:5]) << 32 | tools.byte_to_uint32(data[5:9])) / 1000.0
...
class ModulesStatusCtrl(object):
    ...
    def check_module_enable(self, event_client):
        event_client.event_msg_invalid_check_callback_register(self.check_module_invalid)
    ...
    def check_module_invalid(self, event_msg):
        ...
        if self._check_module_status((event_msg.module_id), rm_define.module_status_offline):
            return True, rm_define.BLOCK_ERR_MODULE_OFFLINE
        elif self._check_module_status((event_msg.module_id), rm_define.module_status_error):
    ...
    def module_status_process(self, event_client, msg):
        ...
        for i in range(online_module_num):
            ...
            if module_status_msg_id in self.MODULE_STATUS_MSG_ID_TO_MODULE_ID_MAP.keys():
                online_module_list.append(self.MODULE_STATUS_MSG_ID_TO_MODULE_ID_MAP[module_status_msg_id])
            msg_num = data[2]
        ...
        offline_module_list = list(set(self.ALL_MODULE_STATUS_MSG_ID) - set(online_module_list))
...
class SensorAdapterCtrl(RobotCtrlTool):
    func_str_dict = {'rm_define.cond_sensor_adapter1_port1_high_event': rm_define.cond_sensor_adapter1_port1_high_event,
    ...
    def event_init(self):
        ...
        for id in range(1, 8):
            for port in range(1, 3):
                self.pulse_event_cb.append('sensor_adapter' + str(id) + '_port' + str(port) + '_high')
                self.pulse_event_cb.append('sensor_adapter' + str(id) + '_port' + str(port) + '_low')
                self.pulse_event_cb.append('sensor_adapter' + str(id) + '_port' + str(port) + '_trigger')
        ...
        resp = self.sensor_adapter.set_sensor_adapter_param(0, 1, **{'set_mask': 2, 'adc_accuracy': 0, 'push_freq': 0,
                                                                     'io_event': 1})
        self.sensor_adapter.set_sensor_adapter_param(0, 2, **{'set_mask': 2, 'adc_accuracy': 0, 'push_freq': 0,
                                                              'io_event': 1})
    ...
    def pulse_event_process(self, event_client, msg):
        ...
        port_num = data[1]
        io_value = data[2]
        logger.info('board_id %d, port_num %d, io_value %d' % (board_id, port_num, io_value))
        ...
        for board in range(1, 8):
            for port in range(1, 3):
                ...
                if board_id == board and port_num == port and io_value == 1 and func_str in self.pulse_event_cb:
                    self.condition_mutex.acquire()
                    self._wakeup_condition_waiting(func_str)
                    self.condition_event_time[func_str] = time.time()
                    self.condition_event_table[func_str] = True
                ...
                if board_id == board and port_num == port and io_value == 0 and func_str in self.pulse_event_cb:
                    self.condition_mutex.acquire()
                    self._wakeup_condition_waiting(func_str)
                    self.condition_event_time[func_str] = time.time()
                    self.condition_event_table[func_str] = True
                ...
                func_str = 'sensor_adapter' + str(board) + '_port' + str(port) + '_trigger'
                if board_id == board and port_num == port and func_str in self.pulse_event_cb:
                    self.condition_mutex.acquire()
                    self._wakeup_condition_waiting(func_str)
                    self.condition_event_time[func_str] = time.time()
                    self.condition_event_table[func_str] = True
    ...
    def register_event(self, func_dict):
        for board in range(1, 8):
            for port in range(1, 3):
                event_str = 'sensor_adapter' + str(board) + '_port' + str(port) + '_high_event'
                sensor_adapter_str = 'sensor_adapter' + str(board) + '_port' + str(port) + '_high'
                if event_str in func_dict.keys():
                    self.event_client.event_callback_register(sensor_adapter_str, func_dict[event_str])
                    logger.info('SENSORADAPTER_CTRL: register event  %s' % sensor_adapter_str)
                event_str = 'sensor_adapter' + str(board) + '_port' + str(port) + '_low_event'
                sensor_adapter_str = 'sensor_adapter' + str(board) + '_port' + str(port) + '_low'
                if event_str in func_dict.keys():
                    self.event_client.event_callback_register(sensor_adapter_str, func_dict[event_str])
                    logger.info('SENSORADAPTER_CTRL: register event  %s' % sensor_adapter_str)
                event_str = 'sensor_adapter' + str(board) + '_port' + str(port) + '_trigger_event'
                sensor_adapter_str = 'sensor_adapter' + str(board) + '_port' + str(port) + '_trigger'
                if event_str in func_dict.keys():
                    self.event_client.event_callback_register(sensor_adapter_str, func_dict[event_str])
                    logger.info('SENSORADAPTER_CTRL: register event  %s' % sensor_adapter_str)

    def get_sensor_adapter_adc(self, sensor_adapter_id, port_num):
        duss_result, resp = self.sensor_adapter.get_sensor_adapter_data(sensor_adapter_id, port_num)
        if duss_result == rm_define.DUSS_SUCCESS:
    ...
    def get_sensor_adapter_pulse_period(self, sensor_adapter_id, port_num):
        ...
        if duss_result == rm_define.DUSS_SUCCESS:
            ...
            logger.info('SENSORADAPTER_CTRL:pulse_peroid is %d' % (pulse_peroid))
            return pulse_peroid
        else:
            return 0
    ...
    def check_condition(self, func_str):
        ...
        event_happen = False
        if func_str in self.condition_event_table.keys():
            adapterid = int(re.findall('\d+', func_str)[0])
            portid = int(re.findall('\d+', func_str)[1])
            if self.get_sensor_adapter_io_level(adapterid, portid) == 1:
                if "high" in func_str:
                    self.condition_event_table[func_str] = True
                elif "low" in func_str:
                    self.condition_event_table[func_str] = False
            elif self.get_sensor_adapter_io_level(adapterid, portid) == 0:
                if "high" in func_str:
                    self.condition_event_table[func_str] = False
                elif "low" in func_str:
                    self.condition_event_table[func_str] = True
    ...
    def check_cond_wait_event(self, func_str):
        logger.info('SENSORADAPTER_CTRL: check cond wait event, func_str is %s' % (func_str))
        event_state = False
        self.condition_mutex.acquire()
        if func_str in self.condition_wait_event_list.keys():
            event_state = self.condition_wait_event_list[func_str].isSet()
...
class IrDistanceSensorCtrl(RobotCtrlTool):
    ...
    def enable_measure(self, port_id):
        ...
        if port_id == 0:
            for port_id in range(1, 5):
                ...
                self.ir_distance_sensor.sub_tof_data_info_push(port_id, self.tof_push_info_process)
        else:
    ...
    def _add_check_condition_table(self, port_id, distance):
        ...
        self.check_condition_table[port_id] = {'distance': distance, 'time': time.time(), 'flag': True}
        self.check_condition_mutex.release()

    def register_event(self, func_dict):
        for (key, value) in func_dict.items():
            if re.match(r'^ir_distance_[1-9]{1}?_(lt|le|eq|ge|gt){1}?_[0-9]*_{0,1}?[0-9]+_event$', key) and callable(
                    value):
    ...
    def check_condition(self, func_str):
        ...
        if port_id in self.check_condition_table.keys():
            ...
            if self.check_condition_table[port_id]['flag']:
                event_happen = self._comp(comp, self.check_condition_table[port_id]['distance'], dist)
        self.condition_mutex.release()
        return event_happen

    def cond_wait(self, func_str):
        if re.match(r'^ir_distance_[1-9]{1}?_(lt|le|eq|ge|gt){1}?_[0-9]*_{0,1}?[0-9]+$', func_str):
            condition_wait_event = threading.Event()
            port_id, comp, dist = self._parse_data(func_str)
    ...
    def check_cond_wait_event(self, func_dict):
        ...
        if func_dict['id'] in self.condition_wait_event_dict.keys():
            for item in self.condition_wait_event_dict[func_dict['id']]:
                if item['attr']['comp'] == func_dict['comp'] and item['attr']['dist'] == func_dict['dist']:
    ...
    def _cond_wait_register(self, func_dict, wait_event):
        ...
        self.condition_wait_event_dict[func_dict['id']].append({'attr': func_dict, 'event': wait_event})
        self.condition_mutex.release()
    ...
    def _comp(self, comp_str, param1, param2):
        if (comp_str == 'lt' and param1 < param2) \
...
class RoboticGripperCtrl(RobotCtrlTool):
    ...
    def open(self, level=None):
        ...
        duss_result = self.robotic_gripper.robotic_gripper_ctrl(1, rm_define.gripper_open,
                                                                self.power_level_mapping[level])
    ...
    def close(self, level=None):
        ...
        if not level:
            level = self.power_level
        self.robotic_gripper.robotic_gripper_ctrl(1, rm_define.gripper_close, self.power_level_mapping[level])
    ...
    def stop(self):
        ...
        duss_result = rm_define.DUSS_SUCCESS
        self.robotic_gripper.robotic_gripper_ctrl(1, rm_define.gripper_stop, self.power_level_mapping[self.power_level])
        self.action_status = 'stop'
        return get_task_dict(None, 'action_immediate', (), result=duss_result)
...
class ServoCtrl(RobotCtrlTool):
    def __init__(self, event_client):
        ...
        self.event_client = event_client
        self.mode = {
            1: rm_define.servo_unknow_mode,
            2: rm_define.servo_unknow_mode,
            3: rm_define.servo_unknow_mode,
            4: rm_define.servo_unknow_mode,
        }
        self.robotic_arm_servo_id_list = []

    def __enable(self):
        for id in range(1, 4):
            self.servo.set_servo_data(id, 0, enable=True)

    def __disable(self):
        for id in range(1, 4):
            self.servo.set_servo_data(id, 0, enable=False)

    @event_register
    def __switch_mode(self, servo_id, mode):
        if servo_id in self.robotic_arm_servo_id_list:
            return get_task_dict(None, 'action_immediate', (servo_id, mode), result=rm_define.FAILURE,
                                 err_code=rm_define.BLOCK_ERR_SERVO_IN_ROBOTIC_ARM)
        if self.mode[servo_id] == mode:
            return get_task_dict(None, 'action_immediate', (servo_id, mode), result=rm_define.SUCCESS)
        else:
            duss_result, resp = self.servo.set_servo_mode(servo_id, mode)
            if tools.duss_result_check(duss_result, resp, duml_cmdset.DUSS_MB_RET_OK) == rm_define.SUCCESS:
                self.mode[servo_id] = mode
                return get_task_dict(None, 'action_immediate', (servo_id, mode), result=rm_define.SUCCESS)
            elif tools.duss_result_check(duss_result, resp,
                                         duml_cmdset.DUSS_MB_RET_SERVO_IN_ROBOTICARM_MODE) == rm_define.SUCCESS:
                logger.warning('CUR SERVO IN ROBOTIC ARM MODE')
                self.robotic_arm_servo_id_list.append(servo_id)
                self.mode[servo_id] = rm_define.servo_position_mode
                return get_task_dict(None, 'action_immediate', (servo_id, mode), result=rm_define.FAILURE,
                                     err_code=rm_define.BLOCK_ERR_SERVO_IN_ROBOTIC_ARM)
            else:
                return get_task_dict(None, 'action_immediate', (servo_id, mode), result=rm_define.FAILURE)
    ...
    def get_angle(self, servo_id):
        if servo_id in self.servo_status.keys():
            return tools.number_mapping(self.servo_status[servo_id]['angle'], 0, 2048, -180, 180)
        else:
            return 0

    @event_register
    def set_angle(self, servo_id, angle, wait_for_complete=False):
        # angle (-180 ~ 180), pre: 0.1
        logger.info('ROBOTICGRIPPER_CTRL: set angle')
        if get_result(self.__switch_mode(servo_id, rm_define.servo_position_mode)):
            ...
            if wait_for_complete == True:
                ...
                duss_result, identify = self.servo.set_servo_angle_task(servo_id, angle)

                if duss_result == rm_define.DUSS_SUCCESS:
                    return get_task_dict(identify, 'task', (servo_id, angle, wait_for_complete), result=duss_result)
                else:
                    self.interrupt_func_unregister()
                    return get_task_dict(None, 'no_task', (servo_id, angle, wait_for_complete), result=duss_result)
            else:
                angle = int(tools.number_mapping(angle, -180, 180, 0, 2048))
                duss_result, resp = self.servo.set_servo_data(servo_id, angle)
                return get_task_dict(None, 'action_immediate', (servo_id, angle, wait_for_complete), result=duss_result)
        else:
            return get_task_dict(None, 'action_immediate', (servo_id, angle, wait_for_complete),
                                 result=rm_define.FAIURE)

    def set_speed(self, servo_id, speed):
        # speed (-294 ~ 294), pre: 1
        if get_result(self.__switch_mode(servo_id, rm_define.servo_speed_mode)):
            speed = int(tools.number_mapping(speed, -294, 294, 0, 900))
            duss_result, resp = self.servo.set_servo_data(servo_id, speed)
    ...
    def stop(self, servo_id=0):
        if servo_id == 0 or servo_id == None:
            for id in range(1, 4):
                self.servo.set_servo_data(id, 0)
        else:
            self.servo.set_servo_data(servo_id, 0)
        return 0
...
class RoboticArmCtrl(RobotCtrlTool):
    ...
    def __fail_reason_to_block_running_result(self, err_code):
        if err_code in self.__err_code.keys():
            return self.__err_code[err_code]
        else:
            return 0
    ...
    def move(self, x, y, wait_for_complete=False):
        ...
        if wait_for_complete == True:
            ...
            duss_result, identify = self.robotic_arm.robotic_arm_move_ctrl(1, rm_define.opposite_move,
                                                                           rm_define.mask_xy, x, y, 0, rm_define.TASK)

            if duss_result == rm_define.DUSS_SUCCESS:
                return get_task_dict(identify, 'task', (x, y, wait_for_complete), result=duss_result,
                                     fail_reason_to_block_running_result_callback=self.__fail_reason_to_block_running_result)
            else:
                self.interrupt_func_unregister()
                return get_task_dict(None, 'no_task', (x, y, wait_for_complete), result=duss_result)
        else:
            duss_result = self.robotic_arm.robotic_arm_move_ctrl(1, rm_define.opposite_move, rm_define.mask_xy, x, y, 0,
                                                                 rm_define.NO_TASK)
            return get_task_dict(None, 'action_immediate', (x, y, wait_for_complete), result=duss_result)
    ...
    def moveto(self, x, y, wait_for_complete=False):
        ...
        if wait_for_complete == True:
            ...
            duss_result, identify = self.robotic_arm.robotic_arm_move_ctrl(1, rm_define.absolute_move,
                                                                           rm_define.mask_xy, x, y, 0, rm_define.TASK)
            ...
            duss_result, resp = self.robotic_arm.robotic_arm_move_ctrl(1, rm_define.absolute_move, rm_define.mask_xy, x,
                                                                       y, 0, rm_define.NO_TASK)
            return get_task_dict(None, 'action_immediate', (x, y, wait_for_complete), result=duss_result)
    ...
    def recenter(self, wait_for_complete=False):
        ...
        if wait_for_complete == True:
            ...
            duss_result, identify = self.robotic_arm.robotic_arm_move_ctrl(1, rm_define.absolute_move,
                                                                           rm_define.mask_xy, 91, 41, 0, rm_define.TASK)

            if duss_result == rm_define.DUSS_SUCCESS:
                return get_task_dict(identify, 'task', (wait_for_complete), result=duss_result,
                                     fail_reason_to_block_running_result_callback=self.__fail_reason_to_block_running_result)
            else:
                self.interrupt_func_unregister()
                return get_task_dict(None, 'no_task', (wait_for_complete), result=duss_result)
        else:
            duss_result, resp = self.robotic_arm.robotic_arm_move_ctrl(1, rm_define.absolute_move, rm_define.mask_xy,
                                                                       91, 41, 0, rm_define.NO_TASK)
            return get_task_dict(None, 'action_immediate', (wait_for_complete), result=duss_result)
...
class SerialCtrl(RobotCtrlTool):
    ...
    def event_init(self):
        ...
        self.serial.set_serial_param(self.baud_rate, self.data_bit, self.odd_even, self.stop_bit, \
                                     self.rx_enable, self.tx_enable, self.rx_buffer, self.tx_buffer)
    ...
    def serial_recv_process(self, event_client, msg):
        ...
        logger.info("msg_type is %d, data_len is %d, msg is :%s" % (msg_type, data_len, msg))
    ...
    def serial_config(self, baud_rate, data_bit, odd_even, stop_bit):
        logger.error('serial_config: baud_rate=%s, data_bit=%s, odd_even=%s, stop_bit=%s' % (
        baud_rate, data_bit, odd_even, stop_bit))
        self.serial.set_serial_param(baud_rate, data_bit, odd_even, stop_bit, \
                                     self.rx_enable, self.tx_enable, self.rx_buffer, self.tx_buffer)
    ...
    def stop(self):
        ...
        self.serial.set_serial_param(self.baud_rate, self.data_bit, self.odd_even, self.stop_bit, \
                                     0, 0, self.rx_buffer, self.tx_buffer)
