...
class EventClient(object):
    ...
    def __init__(self, host_id=rm_define.script_host_id):
        self.debug = False
        self.route_table, _, _ = tools.load_route_table(
            EventClient.DEFAULT_ROUTE_FILE, "scratch_service", "scratch_client"
        )
    ...
    def stop(self):
        ...
        logger.info(
            "EventClient: recv thread alive = " + str(self.recv_thread.isAlive())
        )
        self.socketfd.close()
        ...
        while self.finish == False:
            ...
            if msg["cmd_set"] == duml_cmdset.DUSS_MB_CMDSET_RM:
                if msg["cmd_id"] in self.task_push_cmdid_list:
                    ...
                    task_identify = (
                        str(msg["cmd_set"]) + str(msg["cmd_id"]) + str(msg["task_id"])
                    )
            ...
            if msg["ack"] == True:
                ...
                if identify in self.wait_ack_list.keys():
                    for j in range(0, len(self.wait_ack_event_list)):
                        if (
                            self.wait_ack_event_list[j].valid
                            and self.wait_ack_event_list[j].identify == identify
                        ):
    ...
    def send2myself(self, data):
        try:
            self.socketfd.sendto(
                data.encode("utf-8"), "\0/duss/mb/" + str(hex(self.my_host_id))
            )
        except Exception as e:
    ...
    def async_req_unregister(self, cmd_set_id, cb=None):
        ...
        if cb != None:
            if (
                cmd_set_id in self.async_req_cb_list.keys()
                and cb in self.async_req_cb_list[cmd_set_id]
            ):
    ...
    def event_notify_register(self, event_name, robot_event):
        ...
        if (
            event_name in self.event_notify_not_register_dict.keys()
            and time.time() - self.event_notify_not_register_dict[event_name] < 5
        ):
    ...
    def event_come_to_process(self, event_name, callback_data=None):
        ...
        if event_name in self.event_callback_list.keys():
            self.event_process_list[event_name]["callback"] = self.event_callback_list[
                event_name
            ]
    ...
    def wait_for_event_process(self, func_before_event):
        ...
        for (event_name, callback_items) in self.event_process_list.items():
            if (
                callback_items["callback"] != None
                and callback_items["callback"].__name__ != "dummy_callback"
            ):
                callback_list.append(callback_items)
                self.event_process_list[event_name] = {
                    "callback": None,
                    "callback_data": None,
                }
        self.event_process_mutex.release()
    ...
    def ack_register_identify(self, event_msg):
        self.wait_ack_mutex.acquire()
        identify = (
            str(event_msg.receiver)
            + str(event_msg.cmd_set)
            + str(event_msg.cmd_id)
            + str(event_msg.seq_num)
        )
    ...
    def send_sync(self, event_msg, time_out=duml_cmdset.MSG_DEFAULT_TIMEOUT):
        ...
        if check_result == True:
            logger.warn(
                "RECEIVER %d, MODULE ID %d, OFFLINE OR ERROR %s"
        ...
        if event_msg.cmd_type & duml_cmdset.NEED_ACK_TYPE:
            ...
            if not self.wait_ack_event_list[j].wait_ack_event.isSet():
                ...
                logger.warn(
                    "CMDSET = "
                    + str(hex(event_msg.cmd_set))
                    + ", CMDID = "
                    + str(hex(event_msg.cmd_id))
                    + " TIMEOUT"
                )
    ...
    def send_task_async(
        ...
        if resp["data"][0] == duml_cmdset.DUSS_MB_RET_OK:
            if resp["data"][1] == 0:
                ...
                logger.info(
                    "TASK ID = " + str(event_task["task_id"]) + " ALREADY FINISH."
                )
    ...
    def task_sync_process(self, identify, task_push_msg):
        ...
        logger.info(
            "TASK ID = "
            + str(task_push_msg["task_id"])
            + ", "
            + str(task_push_msg["percent"])
            + "% STATE = "
            + str(task_push_msg["result"])
        )
